...
poc_outcome:
  ...
  evidence: All four AC checks re-verified green at candidate eca4eb82, the cycle-2 validation of the Captain's F2/F3 ruling. kc-dev-flow-contract-test.py PASS exit 0; check-pr-title.test.py PASS exit 0 (13 fixture rows + 3 boundary cases); git diff origin/main -- kc-dev-flow/scripts/check-pr-title.py empty; git grep -n capture-oracle and git grep -niE 'capture.?oracle' both exit 1 over the tracked tree, against 8 hits in 6 files on origin/main; the 13 verdict rows diff empty against origin/main; released body sha256 ea187ab4d1771ce3cb549c2619278a77bc904e39c0fb2b61770ed94d12f5cf57 at 10551 bytes recomputed independently and matching the manifest pin; synced extension block byte-identical to the resource (27521 == 27521 chars). git diff 13543f3c..eca4eb82 --stat confirms exactly the two ruled edits and nothing else: 3 files, 3 insertions, 3 deletions.
  strongest_limit: The deletion is sound; direction is change because the experiment left its POC envelope, not because the candidate is wrong. Decision-ready took 1680s against a 15-minute limit and the Captain intervened mid-flight to amend AC-3 and poc_stop_when, which the receipt anticipated as overrulable to Pilot at the cost of one stage. A second Captain intervention (2026-09-14, ordering the F2/F3 fix) landed after decision-ready and outside any POC feedback context - reproduced that the prove stage's non-report sections break the correction-round authority hash, so the FO applied the fix under direct Captain ruling instead - which strengthens rather than reopens the case that this item outgrew POC. Second limit - re-derivation was not exercised here, so "re-deriving remains possible" rests on scripts/fixtures/release-please-runtime being byte-unchanged from the revision that produced three agreeing runs, not on a fresh run.
...
- DONE: Confirm the two edits are exactly what F2 and F3 proposed and nothing more
  `git diff 13543f3c..eca4eb82 --stat`: 3 files, 3 insertions(+), 3 deletions(-). `release-please-verdicts.tsv:7` line changed from `# not re-derived by this file's capture command.` to `# not part of this file's capture.` — F2's exact wording. `pr-merge-extension.md` and its synced `docs/dev/_mods/pr-merge.md` both changed `this repository's own` to `this repository's root-level` immediately before the `scripts/fixtures/release-please-runtime` path — reads as F3's proposed "prefix the repo-root path ... with 'this repository's root-level'"; resource and mod copy identical (see AC-4 below). No other file touched.
- DONE: Confirm the four acceptance criteria still hold at the new candidate
  AC-1: `git grep -n capture-oracle` and `git grep -niE 'capture.?oracle'` both exit 1; file absent. AC-2: `python3 scripts/kc-dev-flow-contract-test.py` → PASS exit 0. AC-3: `python3 kc-dev-flow/scripts/check-pr-title.test.py` → PASS exit 0 (13 fixture rows + 3 boundary cases); `git diff origin/main -- kc-dev-flow/scripts/check-pr-title.py` empty; the 13 non-comment verdict rows diff empty against `origin/main` (only header-comment lines differ). AC-4: recomputed released-body sha256 `ea187ab4d1771ce3cb549c2619278a77bc904e39c0fb2b61770ed94d12f5cf57` / 10551 bytes matches `contract-manifest.json`'s `pr_merge_released_body` pin; synced extension block byte-identical to the resource (27521 == 27521 chars); oracle section names no re-derivation script (`grep -n capture-oracle` over both copies of `### The title rule's oracle` returns nothing).
- DONE: Updated `## POC outcome` in place (not appended)
  `evidence` and `strongest_limit` fields edited in place to cite candidate `eca4eb82` and the re-verified numbers above; `direction: change` unchanged per the FO's note — both triggers that forced it (decision-ready past the 15-minute limit, and the first Captain intervention) still stand, and this second, post-decision-ready intervention only reinforces that the item outgrew POC. `## POC close measurement` left untouched — still `pending`, no new information about terminal cleanup timing.
  Frontmatter `started:` now carries `2026-09-14T06:35:34.73052Z` (was empty at cycle-1's F1 finding) — F1 appears already resolved; not re-verified against `poc-close-guard.py` since that guard is out of this item's scope.
  worktree clean throughout (`git status` shows only the branch-ahead note, no pending changes) after commit `eca4eb82`.

### Summary

Cycle-2 validation confirms the FO's scoped read: the diff since `13543f3c` is exactly F2 and F3 as the Captain's ruling stated them, nothing else touched, and all four acceptance criteria are still green at `eca4eb82`. `## POC outcome` was updated in place with the new candidate and re-verified figures; `## POC close measurement` stays `pending`. No new findings — F1, F2, and F3 all read as closed.
