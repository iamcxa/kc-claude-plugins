        ...
        - id: gate:q0z8h3xny0qxv0r5srter8tj:release
          ...
          attempts:
            ...
            - id: gate-attempt:q0z8h3xny0qxv0r5srter8tj-release-2
              ...
              resolution:
                type: Resolution
                id: resolution:spacedock:q0z8h3xny0qxv0r5srter8tj:release:2
                briefing: briefing:q0z8h3xny0qxv0r5srter8tj:release:attempt-2:revision-1
                by: person:captain
                at: "2026-08-21T07:20:53.341207Z"
                decision: approve
...
- DONE: Finding D closed — the default-format test now fails when the loader's default format is changed to json, by comparing the default output against an explicit `--format text` run; the RED was observed against that exact mutation and reverted.
  Added a second subprocess run with an explicit `--format text` flag on the same fixture and asserted `text_format.stdout == explicit_text_format.stdout`, keeping the existing header-content assertion after it. Mutated `parser.add_argument("--format", ...)`'s `default="text"` to `default="json"`; the test failed with `the no-flag default output diverged from an explicit --format text run ...: '{"declared_receipts": [...], "loaded": ...}' vs '{"declared_receipts": [...], "logical_stage": "prove", "next_w...'` (exit 1) — the no-flag run collapsed to the single-line JSON document while the explicit `--format text` run still produced the multi-line rendered form, so the two diverged exactly as the finding predicted. Reverted via `cp` from a pre-mutation backup; `diff` against the backup was empty; reran to `PASS`.
- DONE: Finding E closed — the multi-block fixture now interleaves two distinct valid vendored paths (`../../kernel.md` and the sibling `base.md`) across its two blocks, keeping `../../kernel.md` and `receipt_a` each repeated once, and the asserted document order is shown RED against a group-by-path mutation, then reverted.
  Changed the second entry in each of the two JSON blocks from `"path": "../../kernel.md"` to `"path": "base.md"` (root/profiles/poc-exploration/base.md, vendored by the fixture's `write_fixture`), leaving the first entry in each block on `../../kernel.md` and receipt `receipt_a` in both — so the expected sequence `[receipt_a, receipt_b, receipt_a, receipt_c]` is reachable only by in-order per-entry append. Implemented a group-by-path mutation in `check_conditional_references`: entries are now appended into a `dict[str, list[str]]` keyed by the entry's resolved `target` path (insertion-ordered by first occurrence) instead of directly into `declared_receipts`, then flattened group-by-group after the loop. Ran the test against that mutant: failed with `declared_receipts did not preserve exact document order across multiple conditional-references blocks, or dropped the repeated name: ['receipt_a', 'receipt_a', 'receipt_b', 'receipt_c']` (exit 1) — the two `../../kernel.md` entries (both `receipt_a`) were grouped ahead of the two `base.md` entries (`receipt_b`, `receipt_c`), reordering exactly as the finding predicted; with the prior single-shared-path fixture this same mutant would have passed undetected. Reverted via `cp` from a pre-mutation backup; `diff` against the backup was empty; reran to `PASS`.
- DONE: Scope held — Finding C stays unimplemented, the two loader copies stay byte-identical, the path fail-closed cases at profile-contract-loader.test.py:305-429 stay unmodified and green, and no production behaviour changed.
  No change was made to `profile-contract-loader.py` in the committed diff (only the two mutation attempts above, both applied then reverted with an empty `diff` against a pre-mutation backup each time). `diff kc-dev-flow/scripts/profile-contract-loader.py docs/dev/_mods/profile-contract-loader.py` is empty. `git diff 6d0bd41f -- kc-dev-flow/scripts/profile-contract-loader.test.py` shows four hunks, all at line 552 and later — outside 305-429. `python3 kc-dev-flow/scripts/profile-contract-loader.test.py` prints `PASS`. `python3 scripts/kc-dev-flow-contract-test.py` with `spacedock` excluded from `PATH` (its own designed skip path) prints `kc-dev-flow contract: PASS`; with `spacedock` on `PATH` it fails at the same pre-existing, unrelated `profile-spacedock-route` verdict-casing mismatch (`did not terminalize directly`, `verdict: PASSED` vs. the test's lowercase expectation) reported in every prior stage cycle, confirming no new regression. Committed `10edd0a4` on `spacedock-ensign/declared-receipt-has-no-reader` and pushed onto the existing open draft PR #262.

### Summary

Closed both round-2 findings with test-only changes to `profile-contract-loader.test.py`: the text-format test now compares the no-flag default run against an explicit `--format text` run byte-for-byte (Finding D), and the multi-block document-order test interleaves two distinct vendored paths so a group-by-path reader reorders the result (Finding E). Both fixes were proven live by mutation — a `default="text"→"json"` flip and a group-by-path accumulator — each reproducing the predicted failure and reverted with an empty diff. No production file changed; the two loader copies stay byte-identical; the path fail-closed range (305-429) is untouched; the contract test suite's sole failure remains the pre-existing, unrelated `profile-spacedock-route` casing bug. Per the FO's stopping rule, this is the last correction round on this candidate.

## Stage Report: validation (cycle 3)

- DONE: Independently reproduce BOTH round-2 mutations at 10edd0a4 — Finding D's default-format flip (text→json) and Finding E's group-by-resolved-path accumulator — and revert each with `git status --porcelain` empty and the suite green after.
  Finding D: flipped `parser.add_argument("--format", ...)`'s `default="text"` to `default="json"`. Test failed: `the no-flag default output diverged from an explicit --format text run ...` (exit 1) — the no-flag run collapsed to a single-line JSON document while the explicit `--format text` run stayed multi-line. Restored from a pre-mutation backup, `diff` empty, reran GREEN. Finding E: replaced the per-entry `declared_receipts.append(receipt)` with a `dict[str, list[str]]` keyed by resolved `target` path, flattened group-by-group after the loop. Test failed: `declared_receipts did not preserve exact document order ...: ['receipt_a', 'receipt_a', 'receipt_b', 'receipt_c']` (exit 1) — the two `../../kernel.md` entries grouped ahead of the two `base.md` entries. Restored from backup, `diff` empty, reran GREEN. Both reproductions matched the implementer's cycle-3 report verbatim.
- DONE: Confirm no production behaviour drifted across the three correction rounds against origin/main (49e66248, current merge-base).
  `git diff origin/main HEAD --stat`: exactly 3 files — the two byte-identical loader copies (22 insertions/3 deletions each, identical hunks, confirmed by `diff` between them = empty) and `profile-contract-loader.test.py` (214 insertions, one hunk `@@ -450,4 +450,218 @@`). The fail-closed range at lines 305-429 sits inside the pre-existing 453-line file, entirely below the hunk's start line 450 — untouched. `git diff origin/main HEAD --stat -- kc-dev-flow/references/profiles/ kc-dev-flow/references/kernel.md` is empty: no contract declaration content changed.
- DONE: Close the gate — walk every claim in `## Accepted outcome and non-goals` and every `## Acceptance evidence` bullet, name the check SEEN to fail on it; name explicitly any claim with no such check.
  Acceptance evidence: bullet 1 (presence, RED→GREEN) — seen RED in implementation cycle 1 (`did not surface the stage's own receipt name: None`). Bullet 2 (isolation; null exclusion + kernel/base exclusion in one assertion, confirmed by reading the fixture at lines 486-546) — seen RED via two mutants (flatten, null-leak) in implementation cycle 1, re-reproduced in validation cycle 1. Bullet 3 (multi-block document order) — seen RED via three mutants: truncate-to-first and sorted-set (implementation cycle 2, re-reproduced validation cycle 2) and group-by-path (this stage, Finding E). Bullet 4 (text-format default) — seen RED via the default-format-flip mutant (this stage, Finding D). Bullet 5 (path fail-closed regression) — confirmed unmodified above; same suite green.
  Accepted-outcome claims WITH a falsifiable check: the `declared_receipts` field's existence/ordering/non-null/stage-scoping/both-format-visibility/multi-block-accumulation claims all trace to the five acceptance-evidence bullets above.
  Accepted-outcome claims with NO test check, verified only by direct code/grep inspection (named explicitly, not asserted complete): (1) the docstring's bounded-guarantee prose itself — `grep -n "does not verify\|does not evaluate\|does not block\|docstring" profile-contract-loader.test.py kc-dev-flow-contract-test.py` returns zero hits, no test asserts on this text; (2) non-goal "no evaluable trigger" — `grep -n trigger profile-contract-loader.py` shows the only two hits are the docstring's own prose (lines 116, 126), `entry["trigger"]`/`entry.get("trigger")` is read nowhere; (3) non-goal "no stage-exit enforcement gate" — every `raise ContractError` in the file branches on path/schema/list-shape, none on receipt/trigger presence; (4) the current-state claim "kernel.md and base.md never declare that block today" — `grep -rl "kc-dev-flow-conditional-references/v1" kernel.md profiles/*/base.md` returns zero hits (9 stage files do, confirmed) and its hypothetical-future scope statement, both audited by grep, not by a runtime test. Non-goals "no new declaration field/second declaration surface" and "no change to which references each contract declares" are diff-verified (the git diff --stat above touches only the 2 loader files + 1 test file; profiles/*.md and kernel.md are untouched) rather than test-asserted.

### Summary

Independently reproduced both round-2 mutations at HEAD `10edd0a4`: the default-format flip (Finding D) and the group-by-resolved-path accumulator (Finding E) each reproduced the implementer's exact reported failure and were reverted with an empty diff, suite green after each. Confirmed against `origin/main` that the only production change across all three correction rounds is the single 22/3-line reader addition in both byte-identical loader copies, everything since `6d0bd41f` is test-only, and the 305-429 fail-closed range sits outside the test file's one changed hunk. Walked every accepted-outcome claim and acceptance-evidence bullet: five claims trace to a named test seen RED (presence, isolation, multi-block ×3 mutants, text-format); four prose/non-goal claims (the docstring's bounded guarantee, non-evaluable trigger, no enforcement gate, kernel/base's current non-declaration) have no automated check and stand on direct grep/code inspection only, named explicitly rather than folded into "complete." No code changes this stage.

## Stage Report: release (cycle 2)

- DONE: Confirm origin/main..HEAD is exactly the three candidate commits, no unrelated commit crept in across the correction rounds, and required checks are green at that exact delivery revision — no rebase needed.
  `git log origin/main..HEAD` (after `git fetch origin main`) lists exactly `d6619580`, `6d0bd41f`, `10edd0a4` — the original candidate plus both correction-round commits, nothing else. `git merge-base HEAD origin/main` is `49e66248`, the same base the release-cycle-1 report rebased onto; current `origin/main` has advanced to `c8b6d679` (an unrelated `kc-team-ops` PR) but `gh pr view 262` still reports `mergeStateStatus: CLEAN`, `mergeable: MERGEABLE` at head `10edd0a4` — confirming no rebase is required, matching the assignment. `gh api repos/iamcxa/kc-claude-plugins/branches/main/protection` shows the repo's one required status check is `version parity (plugin.json / marketplace.json / codex / README)`; `gh pr view 262 --json statusCheckRollup` shows that check `SUCCESS` at head `10edd0a4`, alongside a non-required `GitGuardian Security Checks` also `SUCCESS`.
- DONE: Bring the PR body up to date with what actually shipped — record both correction rounds, name the six mutants each falsifiable claim was seen RED against, and carry the four claims that stand on code inspection with no automated check.
  Rewrote the PR body via `gh pr edit 262 --body-file`. It now has dedicated sections for correction round 1 (Findings A/B closed, Finding C rejected by the FO and not implemented) and correction round 2 (Findings D/E, each with the exact mutation that exposed the prior test's weakness and the fixture/assertion change that closed it), an enumerated list of the six degenerate-reader mutants (flatten, null-leak, truncate-to-first, sorted-set, group-by-resolved-path, format-flip) each falsifiable claim was seen RED against, and the four prose/non-goal claims with no automated check (docstring bounded-guarantee text, non-evaluable `trigger`, no stage-exit enforcement gate, kernel/base's current non-declaration) named explicitly rather than folded into "complete." Delivery revision, delivery-base reason, and both residuals carry over unchanged in substance from the prior body.
  Verified post-edit: `gh pr view 262 --json headRefOid,mergeable,mergeStateStatus,statusCheckRollup,isDraft` — head still `10edd0a4`, still Draft, `CLEAN`/`MERGEABLE`, required check still `SUCCESS` (body edit does not touch code or re-trigger checks).
- DONE: Both residuals stay in the body, unchanged in substance — declared_receipts ships with no consumer, and kc-dev-flow-contract-test.py fails locally on the pre-existing profile-spacedock-route casing bug. No version bump, no marketplace edit, PR left in Draft.
  The rewritten "Known residuals" section carries both items with the same substance as the prior body (no-consumer accepted at the backlog gate; the `spacedock-route-test-passes-nowhere` casing bug tracked separately). `git diff origin/main HEAD --stat` (re-checked this stage) still shows only the two loader files plus the test file — no `plugin.json`, `.codex-plugin/plugin.json`, or `marketplace.json` touched. `gh pr view 262 --json isDraft` confirms `isDraft: true`; no `gh pr ready` was run.

### Summary

Confirmed the branch needs no rebase: `origin/main..HEAD` is exactly the three candidate commits (`d6619580`, `6d0bd41f`, `10edd0a4`), and PR #262 is `CLEAN`/`MERGEABLE` at that exact head with the repo's one required status check (`version parity`) `SUCCESS`, even though `origin/main` has independently advanced past the branch's merge-base since the prior release cycle. Rewrote the PR body to document both correction rounds (Findings A-E, with D and E's exact discriminating mutations), enumerate the six degenerate-reader mutants each falsifiable acceptance-evidence claim was seen RED against, and name the four prose/non-goal claims that have no automated check. Both residuals and the no-version-bump/no-marketplace-edit/Draft-stays-Draft constraints carry over unchanged.
