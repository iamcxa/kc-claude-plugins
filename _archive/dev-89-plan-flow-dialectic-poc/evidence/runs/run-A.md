...
apply; stop at Pattern 1 per the skill's own instruction ("work through the patterns in order; stop when you
find one that applies").

**Issue cut (3 issues, workflow-step split):**

1. **Write the three ship-flow contract sentences into the runtime README**
   - The problem: DEV-67's three contract sentences are prose in a Spacedock entity, not in ship-flow's own
     README, so nothing next to the runtime states what a candidate is promised.
   - Goal: the ship-flow README states, verbatim, the dispatch-order, isolation, and block-vs-scope-out
     sentences, each attributable to DEV-67.
   - Non-goals: no script enforcement; no change to `without-it.sh` or the review step.
   - Acceptance criteria: AC-1 the README contains all three sentences; AC-2 no existing README section is
     removed to make room.
   - Route-back conditions: if any sentence must change to fit, stop and return to planning.
   - blockedBy: none

2. **Pin the three contract sentences to a contract test**
   - The problem: prose in a README is not machine-checked; a future edit could silently drift the README from
     the runtime behavior it promises.
   - Goal: the ship-flow contract test asserts the README's three sentences are present and unchanged at their
     pinned anchors.
   - Non-goals: does not verify the runtime *behaves* per the sentences — only that the README still states
     them (behavior enforcement is deferred per Alternative A).
   - Acceptance criteria: AC-1 the contract test fails if any of the three sentences is edited or removed; AC-2
     the test passes once Issue 1 lands.
   - Route-back conditions: same as Issue 1.
   - blockedBy: Issue 1

3. **Confirm Kent stops re-asking the three questions across 3 PRs**
   - The problem: the hypothesis's validation measure — zero re-asks in 3 PRs — is only true if actually
     observed, not assumed.
   - Goal: record, for the next 3 ship-flow UATs, whether Kent asks any of the three questions; zero re-asks
     validates the hypothesis.
   - Non-goals: no code change; measurement/observation only.
   - Acceptance criteria: AC-1 three UAT observations recorded; AC-2 an explicit valid/invalidated verdict
     recorded against the validation measure.
   - Route-back conditions: a re-ask routes back to planning as a falsifier hit, per the epic-hypothesis's own
     failure branch.
   - blockedBy: Issue 1, Issue 2

**blockedBy graph:** Issue 1 → Issue 2 → Issue 3 (Issue 3 also directly names Issue 1, transitively).

**Produced → Brief field:** Issue cut (title/count/blockedBy order).

**Verbatim vs. rewritten:** the *splitting decision itself* — which pattern, how many pieces, where the
boundaries fall, and the blockedBy order — transfers usably with **0 sentences rewritten**; this is what AC-1
asks the Issue-cut field to hold. The skill's own per-issue *output template* ("As a [persona], I want to
[action], so that [outcome]" + Given/When/Then), however, does not transfer at all — every issue body above is
written from scratch in this repository's Development Brief shape (problem/goal/non-goals/AC-N/route-back)
instead. This is a wholesale template substitution, not a rewritten sentence inside a field the skill filled; it
is recorded here because it is the largest structural gap found in this run, but it falls outside AC-1's four
named Brief fields (problem, user value, goal+falsifier, issue cut), which concern the *cut*, not the *body
template*.

---

## Project headline (draft, for the FO's Linear write)

> **Ship-flow: enforce the three DEV-67 contract sentences (README + pins)**

Distinct wording from both the `User value:` line (station 2) and the if/then clause (station 3).

---

## AC-1 per-field table (Run A)

| Brief field | Verbatim transfer | Sentences rewritten by FO/worker | New sentences authored (not a rewrite of a skill output) |
|---|---|---|---|
| `## The problem` | I-am/trying-to/but clauses (~2) | 1 (because/impact, grounded from station 2) | 1 (repo-history/prior-art framing) |
| `User value:` | Q1–Q4 answers (verbatim) | 0 | 1 (the one-line reduction itself) |
| `## Accepted outcome` + `Falsifier:` | if/then clause | 2 (Tiny-Acts + Validation-Measures collapsed to prose) | 1 (the `Falsifier:` line) |
| Issue cut (title/count/blockedBy) | pattern choice, count, order | 0 | 0 (per-issue *body template* is a wholesale substitution, tracked separately above, not a field rewrite) |

**Run A / AC-1 verdict:** `## The problem` and `## Accepted outcome` + `Falsifier:` each exceed the falsifier
threshold ("more than one sentence rewritten to fit its field") once new-sentence authorship is counted, so this
run's borrowed-skill outputs do **not** fill those two fields without more than one sentence of change. `User
value:` and the Issue cut stay at or under the threshold. See `.context/dev-89/compare.md` for the run-level
verdict against run B.
