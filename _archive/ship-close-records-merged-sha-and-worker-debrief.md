          ...
          attempts:
            - id: gate-attempt:fgvjsq1wsftn2r6ay4yxp2q1-ideation-1
              ...
              application:
                ...
                state: consumed
        - id: gate:fgvjsq1wsftn2r6ay4yxp2q1:validation
          stage: validation
          attempts:
            - id: gate-attempt:fgvjsq1wsftn2r6ay4yxp2q1-validation-1
              briefing:
                id: briefing:fgvjsq1wsftn2r6ay4yxp2q1:validation:attempt-1:revision-1
                digest: sha256:a46efaf759c51cad50dacdffc338bb9cced67f521fa2d503eb1bf890ee91f77b
                room-ref: ./ship-close-records-merged-sha-and-worker-debrief/review/validation/briefing-1
              resolution:
                type: Resolution
                id: resolution:spacedock:fgvjsq1wsftn2r6ay4yxp2q1:validation:1
                briefing: briefing:fgvjsq1wsftn2r6ay4yxp2q1:validation:attempt-1:revision-1
                by: agent:first-officer
                at: "2026-09-14T14:53:48.265149Z"
                decision: approve
                reason: 'ship FO verification at PR #448 head feeb97ce on macOS: close.test.py all checks passed; 6/189 comment lines; no real repository SHA in fixtures; body carries Candidate, Residuals, without-it, state-branch audit link. Entity pr field still empty — worker to set 448. Approve to done; merge stays with the Captain.'
                conn:
                    quote: 准
                    source: Captain chat 2026-09-14, approving the ship-cloud-wrapper-r3 batch of five (pilot profile)
              application:
...
- DONE: Exact-revision journey evidence at the candidate commit (close.test.py and uat-doc.test.py both pass; AC-1..AC-4 re-verified against the actual diff, not restated from ideation).
  Candidate feeb97ce566b7fb0e33295094c1ff1ff8e053f16 (base 3509e5b0, origin/main; `git merge-tree` clean); `close.test.py`/`uat-doc.test.py` both print "all checks passed" (19 require() assertions in close.test.py); read `resolve_merged_shas`/`_gh_pr_view` (close.py:258-289), `find_debrief_path`/`_shipped_section` (close.py:138-172), and `validate_receipt`'s `MERGED_SHA_RE` gate (close.py:394-429) directly against AC-1..AC-3; `grep -RoE '[0-9a-f]{40}' kc-ship-flow/scripts/fixtures/close-v2/` values fail `git cat-file -t` in this repo (AC-4).
- DONE: Draft PR opened through the pr-merge mod — body has What changed / Evidence / Residuals / without-it unanswered sections and a Candidate line; no merge, no ready; comment lines kept to a minimum (no diff-narration comments); no real SHA from this repository pinned in any fixture (re-verified at this stage).
  PR #448 (iamcxa/kc-claude-plugins), draft, base main, head spacedock-ensign/ship-close-records-merged-sha-and-worker-debrief, `Candidate: feeb97ce566b7fb0e33295094c1ff1ff8e053f16`; https://github.com/iamcxa/kc-claude-plugins/pull/448.
- DONE: Provider feedback disposition recorded (or named as none yet, freshly opened) and remaining obligations/promotion triggers named, including the ideation-stage Work profile receipt `work_profile:` wrapper-key format gap the implementation stage flagged.
  No PR feedback yet — freshly opened Draft, empty population. Remaining obligation: promote only via the pr-merge mod's captain-approved single-PR completion decision (headRefOid==Candidate, mergedAt, feedback fingerprint, required checks). Promotion trigger: pr-merge's sentinel format changing from `pr-merge:N`, or a worker debrief dropping its `## Shipped` heading (entity's Route-back conditions). Carried forward unresolved: the entity's own "## Work profile receipt" YAML still lacks the `work_profile:` wrapper key `surface-map-check.py`'s loader expects — fixing the entity's own body is outside this stage's scope and is named here for the FO.

### Summary

Re-verified AC-1..AC-4 directly against close.py's committed diff (candidate feeb97ce) rather than restating the ideation journey: both test suites pass, the gh-backed merged_sha resolver, the Shipped-section debrief matcher, and validate_receipt's 40-hex gate all read as designed, and no fixture 40-hex value resolves as a real object in this repository. Opened Draft PR #448 through the pr-merge mod's canonical delivery unit with the Captain-directed body sections and a literal `Candidate:` line; no merge or ready-for-review action taken. No PR feedback exists yet on the freshly opened draft; the only carried-forward obligation is the ideation-stage work_profile wrapper-key gap, which remains unaddressed and is named for the FO.
