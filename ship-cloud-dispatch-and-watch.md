...
- DONE: the generated FO boot message carries exactly the three design-spec instructions and nothing that re-implements a dev-flow per-task mechanism
  Ran `dispatch.sh ship-cloud-wrapper --dry-run --workflow-dir <repo>/docs/dev --project-id <dummy>` (read-only against the real `docs/dev`; `--dry-run` prints argv and never calls `conductor workspace create`) and read the generated `ship-cloud-dispatch-and-watch.boot.md`. Its three numbered steps ("Become the first officer... `spacedock claude`... `spacedock state init`"; "Run that entity through its route to a prepared `validation` gate (`spacedock gate prepare`); deliver the Draft PR through the `pr-merge` mod..."; "Stop at the gate. Do not merge. Push every state change...") match `docs/superpowers/specs/2026-09-10-ship-flow-cloud-wrapper-design.md`'s (found on branch `docs/ship-flow-cloud-wrapper-spec`) `boot.md` section verbatim in substance and count — no acceptance, review, disposition, or merge logic is embedded.
