...
Publication uses private temporary files or directories followed by rename. Every `validate`, `append`, `replay`, `show`, `observe`, evidence, usage, and collector file is parsed only from its bounded private snapshot. External appends validate each envelope plus the existing log's immutable run identity, hashes, and contiguous sequence; they do not enforce lane chronology. The shadow collector additionally keeps one process-local verified-prefix cache: each append first fixes one private snapshot of the accepted prefix, then checks that snapshot's complete byte hash, count, and exact run/review/config identity and publishes only from those same bytes. It validates the new event, requires every evidence-bearing subject to match a preflight cache key over the full evidence content hash, closed pointer, and canonical exact evidence identity, and enforces the accepted-log size limit. The cache has no durable or independent authority and disappears with the collector process. An append reserves the run at the PR directory, builds the next complete log privately, and renames it over the accepted log. Concurrent duplicate external appends converge on one accepted event; conflicting or blocked appends do not modify accepted bytes.
...
cross-event ordering. External `append` additionally protects accepted run identity and contiguous sequence,
but can accept envelopes whose lane order is not replayable. The collector's cached append path also binds
the exact verified prefix and preflight evidence cache, but grants no terminal authority. Only `replay` and `show` enforce the
...
| `shadow --observation-file FILE ...` | Best-effort production seam. `--enabled` overrides `KC_PR_FLOW_REVIEW_SHADOW`; enabled collection also requires `--head-check-status` and, when successful, `--live-head`. It accepts exactly one closed `ShadowObservation/v1` (`kc-pr-flow.shadow-observation/v1`) file, preflights its exact evidence once, persists a fresh `run.started`, and appends the deterministic lifecycle through the process-local verified-prefix cache. Before `run.finished` publication it completely replays the pending bytes and rechecks every projected identity against that cache; after publication the observer independently replays once. Every dependency, validation, head, append, or replay failure returns typed `not_observed` and fails open only to the unchanged legacy review. A post-start failure may leave an incomplete non-authoritative run for increment 2.3 recovery. |
