...
## Cross-model arbitration: reconcile by source-set, fail open, never per-finding (2026-06-03)

When adding a third-model tie-breaker to a multi-model review, the cost-and-safety-correct shape is a **single batched arbitration call**, not per-conflict calls. Design rules that survived an adversarial design review: (1) **Blind second model ≠ refutation** — a second model that only emits problems gives no negative evidence; "model B did not flag X" must never demote model A's finding, so reconcile by *divergence*, not by treating silence as a vote. (2) **Reconcile by source-set membership, not a separate unmerged pool** — collators that merge same-fingerprint findings across sources (max-score MULTI-SOURCE) will erase a model's identity; read each merged finding's contributing source set instead of trying to keep parallel lists. (3) **Fingerprint must discriminate issue-type, not line alone** — two distinct bugs at one locus must not collapse; on uncertain match prefer *separate* (false-conflict is cheap, false-agreement hides a bug). (4) **Cap arbitration cost, never silently drop disputes** — a cap on how many disputes get a third-model verdict must exempt direct contradictions and must *list* over-cap disputes with full claim text, never a bare "N dropped" count. (5) **Arbiter is a confidence signal with a visible floor, not a suppressor** — a "false positive" verdict may demote but never hide a finding; high-severity demotions require explicit human ack. (6) **Parse arbiter output strictly and fail open** — accept only known dispute ids (injection-safe against ids echoed from diff content), first-wins on duplicates, map missing/invalid to no-change, and if fewer than half parse, treat the whole arbitration as failed and leave confidence untouched. **Rule**: put the deterministic set-algebra + output-parsing in a unit-tested helper (the semantic fingerprint *assignment* stays agent-side and untestable); keep the human confirmation gate as the final authority; append a "multiple LLMs agreeing is a mild homogenized-lens risk — the human decides" caveat on convergence so false convergence stays recoverable.
