...
review_runtime_timing_state_valid() {
  ...
  jq -e '
    ...
    def phase: . == "identity_and_plan" or . == "inventory" or
      ...
      . == "confirmation_ready";
    exact_keys(["content_sha256","marks","mode","review_key","schema","start_ms"]) and
    .schema == "kc-pr-flow.review-timing-state/v2" and (.review_key | hash64) and
    (.content_sha256 | hash64) and (.mode | mode) and (.start_ms | safe_int) and
    (.marks | type == "array" and length <= 7 and all(.[];
      exact_keys(["monotonic_ms","phase"]) and (.monotonic_ms | safe_int) and (.phase | phase))) and
    ...
    ([.start_ms] + [.marks[].monotonic_ms] | . == sort)
...
review_runtime_timing_start() {
  local review_key="$1" mode="$2" output_file="$3" start_ms without_hash state
  [[ "$review_key" =~ ^[0-9a-f]{64}$ ]] || return 3
  case "$mode" in initial | delta | resolve) ;; *) return 3 ;; esac
  start_ms="$(review_runtime_monotonic_ms)" || return 69
  review_runtime_positive_safe_integer "$start_ms" || return 3
  without_hash="$(jq -S -c -n --arg review_key "$review_key" --arg mode "$mode" \
    --argjson start_ms "$start_ms" '{schema:"kc-pr-flow.review-timing-state/v2",
      review_key:$review_key,mode:$mode,start_ms:$start_ms,marks:[]}')" || return
...
review_runtime_timing_mark() (
  local timing_file="$1" phase="$2" lock snapshot_dir snapshot state now_ms expected_index
  ...
  now_ms="$(review_runtime_monotonic_ms)" || return 69
  review_runtime_positive_safe_integer "$now_ms" || return 3
  [ "$now_ms" -ge "$(jq -r '[.start_ms,.marks[].monotonic_ms] | max' "$snapshot")" ] || return 3
  without_hash="$(jq -S -c --arg phase "$phase" --argjson now_ms "$now_ms" \
    'del(.content_sha256) | .marks += [{monotonic_ms:$now_ms,phase:$phase}]' "$snapshot")" || return
...
review_runtime_external_wait_valid() {
  jq -e '
    def exact_keys($required): type == "object" and (keys | sort) == ($required | sort);
    def safe_int: type == "number" and floor == . and . >= 0 and . <= 9007199254740991;
    exact_keys(["hosted_ci","human_wait","schema","unrelated_queue"]) and
    .schema == "kc-pr-flow.external-wait-attribution/v1" and
    (.hosted_ci | safe_int) and (.human_wait | safe_int) and (.unrelated_queue | safe_int)
  ' "$1" >/dev/null 2>&1
}

review_runtime_timing_finish() (
  local timing_file="$1" lane_file="$2" output_file="$3" external_file="${4-}"
  local snapshot_dir state lanes attribution_snapshot='' external receipt
  snapshot_dir="$(review_runtime_private_snapshot_dir)" || return 74
  state="$snapshot_dir/timing-state.json"
  lanes="$snapshot_dir/lane-durations.json"
  trap 'review_runtime_remove_private_snapshot_dir "$snapshot_dir" "$state" "$lanes" "$attribution_snapshot"' EXIT
  ...
  external='{"hosted_ci":null,"human_wait":null,"unrelated_queue":null}'
  if [ -n "$external_file" ]; then
    attribution_snapshot="$snapshot_dir/external-attribution.json"
    review_runtime_snapshot_regular_file "$external_file" "$attribution_snapshot" \
      'external wait attribution' 1048576 || return
    review_runtime_json_has_unique_members "$(cat "$attribution_snapshot")" >/dev/null 2>&1 || return 3
    review_runtime_external_wait_valid "$attribution_snapshot" || return 3
    external="$(jq -S -c 'del(.schema)' "$attribution_snapshot")" || return
  fi
  receipt="$(jq -S -c -e -n --slurpfile state "$state" --slurpfile lanes "$lanes" \
    --argjson external "$external" '
    $state[0] as $s | $s.marks as $m |
    ($m[0].monotonic_ms - $s.start_ms) as $identity |
    ($m[1].monotonic_ms - $m[0].monotonic_ms) as $inventory |
    ($m[2].monotonic_ms - $m[1].monotonic_ms) as $required |
    ($m[3].monotonic_ms - $m[2].monotonic_ms) as $collector |
    ($m[4].monotonic_ms - $m[3].monotonic_ms) as $targeted |
    ($m[5].monotonic_ms - $m[4].monotonic_ms) as $collation |
    ($m[6].monotonic_ms - $s.start_ms) as $wall |
    ([$external.hosted_ci,$external.human_wait,$external.unrelated_queue] |
      if all(.[]; type == "number") then add else 0 end) as $external_wait |
    ($wall - $identity - $inventory - $collector - $external_wait) as $review |
    select($review >= 0) |
    {
      ...
      attribution_ms:{agent_critical_path:$required,collector:$collector,
        hosted_ci:$external.hosted_ci,unrelated_queue:$external.unrelated_queue,
        human_wait:$external.human_wait},
...
review_runtime_main_timing_finish() {
  local timing_file='' lane_file='' output_file='' external_file=''
  while [ "$#" -gt 0 ]; do
    case "$1" in
      --timing-file | --lane-durations-file | --external-attribution-file | --output)
        [ "$#" -ge 2 ] || return 2
        case "$1" in
          --timing-file) timing_file="$2" ;;
          --lane-durations-file) lane_file="$2" ;;
          --external-attribution-file) external_file="$2" ;;
          --output) output_file="$2" ;;
        esac
        shift 2
        ;;
  ...
  review_runtime_timing_finish "$timing_file" "$lane_file" "$output_file" "$external_file"
}
