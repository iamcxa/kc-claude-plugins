...
# shellcheck disable=SC2030,SC2031 # Intentional per-probe state-root subshells.
...
nonstart_first="$(rehash_event "$(jq -c '.run_id="run-first-nonstart" | .event_type="lane.started" | .sequence=1' <<<"$fresh_fixture")")"
...
duplicate_sequence="$(rehash_event "$(jq -c '.event_type="lane.started" | .sequence=2 | .payload={}' <<<"$fresh_fixture")")"
gap_sequence="$(rehash_event "$(jq -c '.event_type="lane.finished" | .sequence=4 | .payload={}' <<<"$fresh_fixture")")"
...
dead_owner_candidate="$(rehash_event "$(jq -c '.event_type="lane.started" | .sequence=3 | .payload={}' <<<"$fresh_fixture")")"
...
active_owner_candidate="$(rehash_event "$(jq -c '.event_type="lane.finished" | .sequence=4 | .payload={}' <<<"$fresh_fixture")")"
...
malformed_four="$(rehash_event "$(jq -c '.event_type="lane.started" | .sequence=4 | .payload={}' <<<"$fresh_fixture")")"
...
for forbidden_command in resume gc authorize post show; do
