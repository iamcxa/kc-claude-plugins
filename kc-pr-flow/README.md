...
| `kc-pr-review` | `review pr`, PR number/URL, `--full-pass`, `--pass-all`, "8-pass review", `--codex`, "codex review", "second opinion", `minimum-stack`, `without-it` | Agent-dispatched inline code review with optional 8-pass coverage, optional Codex cross-model second opinion, typed exact-head coverage and verdict eligibility, mandatory confirmation, confidence calibration, doc-consistency pre-scans, optional architecture diagrams, and a review-only minimum-stack POC |
