        ...
        - id: gate:kmt1mgvwqy0g9qmshemqtsn1:validation
          ...
          attempts:
            - id: gate-attempt:kmt1mgvwqy0g9qmshemqtsn1-validation-1
              ...
              resolution:
                type: Resolution
                id: resolution:spacedock:kmt1mgvwqy0g9qmshemqtsn1:validation:1
                briefing: briefing:kmt1mgvwqy0g9qmshemqtsn1:validation:attempt-1:revision-1
                by: person:captain
                at: "2026-09-04T04:17:20.418973Z"
                decision: approve
...
- DONE: The trim lands exactly as the cycle-3 shape derived it, numbers measured on the branch.
  `git diff --stat 4bc79749 HEAD` (commit `6a0154e9`) -> 13 files, 685 insertions(+), 25 deletions(-) — under the 700/14-file guard. `wc -l kc-plugin-forge/reference/skill-runner.py` -> 349 (guard 370). Cloud path (`resolve_project_id`+`reconcile_workspace`+`run_cloud`+`page_until_token`, lines 190-303) -> 114 (guard 130). `grep -rn "file_unchanged|host_side_baseline" kc-plugin-forge/` -> zero matches (removed from runner, scenario format, and both T1/T2 scenario files). `skill-runner.sh` absent from the tree and from every grep for callers.
- DONE: AC-5's deliverable exists and AC-4/AC-5 are enforced by G4/G5, each reddens on its named mutation, whole test passes.
  `SKILL.md:610-638` Phase 4 report block carries `Phase 2 Runner: <cloud|bare> (...)`, `Model pin: <model>`, `Scenarios: <path> | hand-designed (...)`, `<ID>: RED <outcome> / GREEN <outcome>`. Baseline: `python3 kc-plugin-forge/scripts/forge-phase2-runner-contract.test.py .` -> `PASSED: 0 violation(s)`. G4 falsifier (delete the `Model pin:` line) -> `FAIL: G4 ... block does not name a runner field ... and a model-pin field`. G5 falsifier (`<passed|failed|judged|error>` -> `<passed|failed|error>`, dropping `judged`) -> `FAIL: G5 ... does not carry per-scenario RED/GREEN outcome labels ...`. Both reverted (`git status --porcelain` clean after), full test re-passes `PASSED: 0 violation(s)`.
- FAILED: AC-2 is not re-observed on the cloud runner this cycle — blocked by the cloud credential, not carried over from the untrimmed runner.
  Verified myself, not reused from the dispatch note: `conductor auth whoami` -> exit 3, `Verification  ✗ token rejected (Unauthorized client request)` (`Token source  CONDUCTOR_API_KEY env var`). Per the FO's fallback instruction, ran the `bare` runner once each for T4 RED and T4 GREEN on the trimmed `skill-runner.py` (ANTHROPIC_API_KEY resolved via `~/.claude/kc-plugins-config/forge.yaml`; model `sonnet-4-6`/resolved `claude-sonnet-4-6`):
  - T4 RED: `outcome=fail runner=bare` — matches the designed RED-fail split (recommended Pilot, not Production; $0.022, 19s).
  - T4 GREEN: `outcome=fail runner=bare` — not the designed pass, but not a runner defect: the model correctly applied the skill's Production-compatibility trigger (`recommended: production`, cites the rule, pushes back to the operator in `operator_note`) but wrote `selected: poc-exploration` echoing the operator's literal choice — the scenario's `file_matches` assertion reads the `selected` field, which the real `kc-dev-flow-work-profile/v3` schema (the one this entity's own receipt uses) reserves for the actual chosen value, distinct from `recommended`. A scenario-assertion field mismatch, same family as the already-recorded T2 RED path-divergence — worth a scenario-design look, not fixed here.
  This closes residual 3: `run_bare` had never completed a scored run before this cycle; it now has two. T1 was not run — the checklist's "T1 may run" is contingent on the cloud batch, which is blocked; a solo bare T1 run was not requested. AC-2's cloud claim (T2/T4 RED-fail, GREEN-pass on all three) stands only as previously recorded, pre-trim, in the existing `## Stage Report: implementation` table — it is NOT re-verified on the trimmed runner this cycle; that gap is the residual left open, to close once the cloud credential is restored. `## Accepted outcome`'s last sentence was already corrected in the entity before this stage report (reads "RED fails on T2 and T4, GREEN passes on all three ...") — verified present, no further edit made since it describes the cloud claim, not today's bare fallback. The cold-start-race fix (residual 1) is in place from commit `6a0154e9` (`page_until_token` requires >=1 non-`userMessage` before `idle` counts as a stall; `reconcile_workspace`'s lock-file fast path waits for `ready`); its one-session falsifier (create a session against a workspace confirmed `sleeping`) needs the cloud runner and is not re-exercised this cycle for the same credential reason.

### Summary

Verified the cycle-2 trim on the branch: 685 insertions/25 deletions vs the 700/14-file guard, `skill-runner.py` at 349 lines (guard 370), cloud path at 114 lines (guard 130), `file_unchanged`/`host_side_baseline` and `skill-runner.sh` fully removed. Confirmed AC-5's report fields exist in `SKILL.md` and that G4/G5 each reddens on its named mutation with the whole contract test passing (0 violations) on the built tree. AC-2 on the cloud runner is BLOCKED this cycle by a rejected `CONDUCTOR_API_KEY` (re-verified directly, not assumed) — as directed, ran the `bare` fallback for T4 RED/GREEN once each: RED reproduces the designed fail, GREEN fails for a scenario-assertion reason (checks `selected` where the model correctly wrote `recommended: production` per the real schema) rather than a runner defect. This closes residual 3 (`run_bare` now has two completed scored runs) but leaves AC-2's cloud re-observation open pending credential recovery.

## Stage Report: implementation (cycle 3)

- DONE: `skill-runner.py`'s cloud credential precondition probes real authentication instead of `CONDUCTOR_API_KEY` presence, fails loudly naming the rejection, and records the resolved token source.
  Cycle-3's first (dead) worker left `probe_cloud_auth()` (shells out to `conductor auth whoami`) threaded through `run_cloud`'s return tuple as a 4th element — functionally correct (`ast.parse` clean, verified this cycle against the real binary in both states below) but it pushed `git diff --stat 4bc79749 HEAD` to **704 insertions**, 4 over the 700 guard (file-level and cloud-path guards were still met: 368/127). Moved the probe to `main()` — called once, before `run_cloud`, which reverts to its pre-probe 3-tuple return — and shortened `probe_cloud_auth`'s docstring; this removes the tuple-size propagation through `run_cloud`'s four return statements entirely rather than trimming prose elsewhere. Own-shell verification (not reused from the dispatch note): `conductor auth whoami` with the inherited env untouched → `Verification  ✗ token rejected (Unauthorized client request)` / `Token source  CONDUCTOR_API_KEY env var`; `env -u CONDUCTOR_API_KEY conductor auth whoami` → `Verification ✓ authenticated` / `Token source     macOS keychain (com.conductor.cli)`. Unit-checked `probe_cloud_auth()` against both real outputs (mocked `subprocess.run` first, then the actual binary via `importlib`) — parses `Token source` and `Verification` correctly in both. All four cloud runs below carry `token_source=macOS keychain (com.conductor.cli)` in the outcome line, confirming it reaches the recorded output, not just the return value. Commit `d4064f55`.
- DONE: T4's assertion targets `recommended:`, the field `choose-work-profile` actually writes; the RED transcript was re-scored against the new assertion before landing it, and RED still fails.
  Could not locate cycle-2's own bare T4 RED transcript on disk (its stage report cites cost/duration, not a saved path; the only `/tmp` files matching the name — `t4-red-full.json`/`t4-green-full.json`, dated 2026-09-03 21:06 — predate cycle 2's 2026-09-04 10:56 CST run and are cloud-shaped transcripts, most likely the pre-trim POC's, not cycle 2's bare fallback). Rather than score an artifact of uncertain provenance, ran fresh bare T4 RED and GREEN against the new `recommended:\s*production` assertion before committing the scenario edit: RED → `outcome=fail` (`recommended: Pilot`), GREEN → `outcome=pass` (`recommended: production`) — discrimination preserved, not loosened, matching the cycle-2 diagnosis (GREEN correctly applies the Production-compatibility trigger; `selected:` is the schema's Captain-authority field, not the skill's to fill deterministically). Both the old and the ambiguous-provenance `/tmp` transcripts were also re-scored against the new assertion for cross-check: same RED-fail/GREEN-pass split. Commit `aafa538b`.
- DONE: AC-2 re-observed on the trimmed runner in the cloud; guards re-measured after both edits above.
  Credential confirmed restored and precondition confirmed fail-loud-and-record before spending any cloud call (bullet 1). Ran all four cloud calls with `env -u CONDUCTOR_API_KEY` (the inherited shell carries the same stale, rejected var the dispatch note predicted), read via `page_until_token`'s `conductor session message --offset` paging (never the sql view):
  - T4 RED: `outcome=fail runner=cloud ... token_source=macOS keychain (com.conductor.cli)`
  - T4 GREEN: `outcome=pass runner=cloud ... token_source=macOS keychain (com.conductor.cli)`
  - T2 RED: `outcome=fail runner=cloud ... token_source=macOS keychain (com.conductor.cli)`
  - T2 GREEN: `outcome=pass runner=cloud ... token_source=macOS keychain (com.conductor.cli)`
  Matches the accepted outcome's cloud claim (RED fails on T2 and T4, GREEN passes on all three) for the two scenarios AC-2 requires this cycle. T1 not re-run: its gate-3 disposition (RED passes on its own) is unchanged and this cycle's checklist scopes AC-2 re-observation to T2/T4; no cloud call was spent on it. The four calls ran concurrently against one project (`kc-claude-plugins`, resolved via `git -C kc-dev-flow remote get-url origin`) and shared one workspace through `reconcile_workspace`'s lock file (`forge-cloud-ws-f39075682765.json`, one `workspaceId` for all four) — this exercises the lock-contention path but not the specific single-session-vs-confirmed-`sleeping`-workspace falsifier the residual-1 docstring names; none of the four stalled or false-idled, consistent with the guard holding, but that specific falsifier is recorded as not separately constructed this cycle (same disposition as cycle 2, cost/time bounded rather than pursued further). Guards re-measured on the built tree after both fixes: `git diff --stat 4bc79749 HEAD` → **13 files, 700 insertions(+), 25 deletions(-)** (exactly at the guard, 0 lines of margin); `wc -l kc-plugin-forge/reference/skill-runner.py` → **364** (guard 370, 6 lines of margin); cloud path (`resolve_project_id`+`reconcile_workspace`+`probe_cloud_auth`+`run_cloud`+`page_until_token`, lines 191-317) → **127** (guard 130, 3 lines of margin). `python3 kc-plugin-forge/scripts/forge-phase2-runner-contract.test.py .` → `PASSED: 0 violation(s)`.

### Discrepancy for the Captain (flagged, not edited)

`## Acceptance criteria` AC-2 still reads "RED fails on T1 and T4"; the gate-3 ruling retired T1 from that claim and both `## Accepted outcome` and this cycle's own scoring already use T2 and T4. Per the dispatch note, scored against the ruling and left the AC-2 sentence untouched — acceptance-criteria text is Captain authority.

### Known limit, stated

No contract-test guard reddens if `run_cloud`'s auth check regresses from real-authentication back to `CONDUCTOR_API_KEY`-presence — this cycle's fix is runtime behavior plus a fresh manual probe, not a new G-numbered guard in `forge-phase2-runner-contract.test.py`. Not added this cycle: out of the checklist's three items, and G1-G5 guard the Phase 4 report template and AC-4/AC-5, not the cloud auth precondition's shape.

### Report-only facts (unchanged from the dispatch note)

- Branch is 3 commits behind `origin/main` (`d98f40b5`, `216a29bc`, `f6695c04`) — confirmed via `git fetch origin main` + `git log --oneline HEAD..origin/main`. Not rebased, per instruction.
- `run_bare` closed its "never completed a run" residual in cycle 2 (two scored runs); this cycle added two more fresh bare runs (T4 RED/GREEN) plus four cloud runs, all scored.
...
- DONE: `kc-dev-flow/scripts/surface-map-check.py` passes against the candidate diff and a durable Evidence file, checking every non-test changed file; each SURFACE line names an acceptance criterion, a falsifier, a safety boundary, or a lifecycle obligation, and its without-it command and removed variant bind that file's path.
  Evidence committed at `docs/dev/.spacedock-state/dev-80-forge-clean-runner/evidence/surface-map-evidence.md`. Ran (this session, from the code worktree at `aafa538b`):
  `python3 kc-dev-flow/scripts/surface-map-check.py 4bc79749 aafa538b <evidence-file> --work-item <entity-file> --brief <entity-file> --repo <worktree>`
  → `excluded: kc-plugin-forge/scripts/forge-phase2-runner-contract.test.py` then `surface-map-check: OK (12 files checked)`, exit 0. `--work-item`/`--brief` both point at this entity file (its `## Work profile receipt` resolves `pilot-product-slice`; its `## Acceptance criteria` supplies the AC-1..AC-5 headings the checker validates targets against). Falsified twice, not merely asserted: (a) dropped the `skill-runner.py` SURFACE line from a scratch copy → `FAIL: missing SURFACE line: kc-plugin-forge/reference/skill-runner.py`, exit 1; (b) replaced the `.github/workflows/marketplace-parity.yml` removed-variant with `true` → `FAIL: without-it pair does not bind .github/workflows/marketplace-parity.yml`, exit 1. Both reverted (scratch copies, entity evidence file untouched by the mutation).
- DONE: Every surface whose absence breaks none of an AC/falsifier/safety-boundary/lifecycle obligation is removed; none qualified. The two candidates the dispatch note named as most likely are refuted by grep against the live tree, not assumed:
  `kc-plugin-forge/reference/clean-profile-test.sh` — NOT orphaned. `grep -n clean-profile-test kc-plugin-forge/reference/skill-runner.py` → line 182, `run_bare`'s subprocess call (the AC-2 bare-fallback path, exercised in cycles 2 and 3 with real scored runs). It is also still called directly from `SKILL.md:347`'s Phase 2.5 smoke step. Two independent callers, both retained; without-it: `grep -n SKILL_RUNNER_MODEL kc-plugin-forge/reference/clean-profile-test.sh` shows the env-var extension `skill-runner.py` depends on for the model pin and stream-json transcript.
  `kc-plugin-forge/reference/parallel-forge.md` — NOT orphaned. `grep -n parallel-forge kc-plugin-forge/skills/kc-plugin-forge/SKILL.md` → three call sites (Phase 2 `--parallel` teammate dispatch, Phase 2.5's bare-process note, Phase 3 `--parallel` teammate dispatch), and its own diff (git diff 4bc79749 aafa538b -- kc-plugin-forge/reference/parallel-forge.md) shows it was rewired, not merely touched: the teammate template now hands every scenario variant to `skill-runner.py` instead of running RED/GREEN in-session — exactly what AC-1's G1/G3 guards check on this file (`forge-phase2-runner-contract.test.py`'s own span list names `reference/parallel-forge.md:30-92`). Cutting it would leave the parallel dispatch path with no template and no falsifier coverage over the parallel span.
  No other retained surface among the 12 was close to the line; each is either a direct AC-1 span the CI-wired contract test already exercises (`SKILL.md`, `parallel-forge.md`, `clean-profile-test.sh`, `skill-runner.py`, the CI workflow file), an AC-4 scenario-format surface the six live AC-2 sessions consumed (`skill-scenarios.md`, both `.scenarios.yaml` files), a safety-boundary the shape itself named (`kc-plugin-forge-sanitize-check/SKILL.md`'s glob extension for `skill-scenarios/*.yaml`), or a lifecycle disclosure obligation from the shape's own "Machine dependencies to declare" / `project_context: update` blocks (`README.md`'s PyYAML note, `architecture.md`/`commands.md`'s Phase 2 description correction).
- DONE: Stop numbers re-measured at the unchanged tip (no code edited this cycle — no removal survived scrutiny, so there was nothing to trim) and the declared checks re-run green.
  `git status --porcelain` → clean at `aafa538b` throughout. `git diff --stat 4bc79749 HEAD` → **13 files, 700 insertions(+), 25 deletions(-)** (unchanged from cycle 3, 0 lines of margin against the Captain's 700 stop-loss). `wc -l kc-plugin-forge/reference/skill-runner.py` → **364** (guard 370). Cloud path (`resolve_project_id` 191–`page_until_token` end 317, `main` starts 318) → **127** lines (guard 130). Re-ran: `python3 kc-plugin-forge/scripts/forge-phase2-runner-contract.test.py .` → `PASSED: 0 violation(s)`; `bash scripts/skill-frontmatter-lint.sh` → 44/44 valid; `bash scripts/marketplace-verify.sh` → L0/L1/L2 PASS for all 7 plugins; `bash scripts/version-parity-check.sh` → all 7 plugins consistent; `bash kc-plugin-forge/scripts/plugin-release-contract.test.sh` → 11 passed, 0 failed; `bash kc-plugin-forge/scripts/plugin-release-contract-check.sh --repo .` → PASS. A diff-scoped grep of the new evidence file against the sanitize-check Class-1 literal list found nothing.
