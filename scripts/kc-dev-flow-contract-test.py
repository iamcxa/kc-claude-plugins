...
# Never quote the bare end-marker text inline within the extension's own
# block: a bare text search for it (the adopter sync instruction is prose,
# not a script) would stop at the first occurrence instead of the actual
# marker. Bounded to [:mod_extension_end] -- the real marker always comes
# first, so a quotation in the adopter-owned region after :end cannot cause
# that failure and must stay outside this check's scope.
require(
    pr_merge_mod[:mod_extension_end].count(pr_merge_extension_end_marker.rstrip("\n")) == 1,
    "docs/dev/_mods/pr-merge.md quotes its own runtime extension end marker "
    "somewhere other than the marker line itself, within the extension's own "
    "block -- never quote it inline",
)
