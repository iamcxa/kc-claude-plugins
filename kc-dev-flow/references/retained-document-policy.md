...
| `implementation` | Apply Rules 1–3 and 6–8 to every retained document the task touches. For an addition or deletion, execute Rule 4. For a deletion, also execute Rule 5 and record where each block landed. |
| `validation` | Spot-check the checks. Take claims from the changed documents and run the check each one names; a claim whose named check does not run, or does not distinguish true from false, is a finding. For an addition or deletion, repeat Rule 4 independently. For a deletion, also attempt Rule 5 independently — try to find a block the implementer missed. |
| `done` | None. |
...
2. List its local path on `ideation`, `implementation`, and `validation`.
