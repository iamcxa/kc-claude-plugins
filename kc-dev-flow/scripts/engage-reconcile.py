...
def by_source(items: object, label: str) -> dict[str, dict[str, object]]:
    ...
    for position, item in enumerate(items):
        ...
        if (
            ...
            or any(not isinstance(value, str) or not value.strip() for value in non_goals)
...
def main() -> int:
    args = parse_args()
    try:
        snapshot = by_source(read_items(args.snapshot, "snapshot"), "snapshot")
        current = by_source(read_items(args.current, "current"), "current")
    ...
    moved = sorted(
        ...
        if any(
            comparable(snapshot[source], field) != comparable(current[source], field)
