...
- DONE: Land one necessity vocabulary across kernel.md and reverse-recovery-audit.md (AC-1, AC-2, AC-3) with contract-test and ablation assertions that reject each renamed boundary for its named reason (AC-5), running the old assertion against each mutation before removing it so no previously caught regression is lost.
  No prior assertion was removed or renamed — every existing `require()` phrase in `scripts/kc-dev-flow-contract-test.py` still matches unchanged text, so the AC-5 pairing table is empty by construction (nothing to lose). New: `reverse-recovery-audit.md`'s `REQUIRED` row now maps to `kernel.md`'s four minimal-necessity reasons plus a named consumer, with a verbatim worked case ("a safety-boundary surface with no observed consumer classifies `REQUIRED`") and a `disproof_hook` search-tier/execution-tier tier statement; `kernel.md` gained "Grade removals too." grading a removed surface by the same reasons as a retained one (AC-3). Two new ablation mutants added and REJECTED: `removal-grading-clause-removed` (kernel), `audit-worked-case-classification-reverted` (audit); commit `17184003` on this branch.
- DONE: Implement semantics_unchanged (required at ideation) and the two equivalence-evidence scalars (required at validation) as loader-required fields refused via ContractError, with the refusal observed in profile-contract-loader.test.py (AC-4), and verify the three fixture builders the shape marked unverified (multi-profile-gate, profile-spacedock-route.test, contract-manifest).
  `profile-contract-loader.py` `resolve_work_item` requires `semantics_unchanged: true|false` for Pilot/Production v3 non-recovery receipts entering `ideation`, and `equivalence_instrument` + `equivalence_instrument_failure` at `validation` when `semantics_unchanged: true`; both raise `ContractError`. `profile-contract-loader.test.py` drives three refusal/load cases (no instrument, placeholder failure value, loads clean at `ideation`) plus two AC-7 backward-compat loads (no key, at `implementation` and `validation`) — all via `resolve_work_item` direct calls, all pass. Ablation mutant `necessity-instrument-field-removed` deletes a field from `NECESSITY_FIELDS` and is REJECTED. Fixture builders: `kc-dev-flow-multi-profile-gate.py`'s `entity_body` needed the new field (fixed, byte-ceiling re-verified — see below); `profile-spacedock-route.test.py` passed unchanged; `contract-manifest.json` needs no change (no reference file added or removed; `kc-dev-flow-contract-test.py`'s `expected_manifest_resources` check still passes).
- DONE: Stay inside the shape's stops (16 files / 500 lines / 180 lines across the two test files against f9683a33); at implementation exit record the diff counts as diagnostics, the comment pass (blocks cut and candidates kept with reasons), the AC-7 adopter migration entry and version, and the roborev observation only if the loader emits implementation_exit_observation_declared true.
  `git diff --stat f9683a33`: 13 files changed, 198 insertions(+), 6 deletions(-) — under the 16-file/500-line stops. The named pair (`profile-contract-loader.test.py` + `kc-dev-flow-minimal-stack-ablation.test.py`): 87+20=107 changed lines — under the 180-line stop. Comment pass: one 2-line comment added (`profile-contract-loader.test.py`, explaining the AC-7 backward-compat intent of two adjacent empty-`necessity_fields` fixtures the code alone doesn't state); kept for that reason; nothing cut. AC-7 migration: `kc-dev-flow/MIGRATION.md` § "2026-09-02 — Pilot and Production ideation declares `semantics_unchanged`", schema stays `kc-dev-flow-work-profile/v3` (no version bump — the string appears in ten files at the delivery base and a bump buys nothing the stage gate does not). `implementation_exit_observation_declared` for this work item at this stage: `True` (confirmed via `profile-contract-loader.py load_contracts` against the dispatched work item — `logical_stage=build`, non-POC profile, no `review_risks` key so `!= ["none"]` is `True`) — the RoboRev observation applies.

### Diagnostics

- `kc-dev-flow/references/kernel.md` byte-ceiling headroom (`scripts/kc-dev-flow-multi-profile-gate.py`'s 40000-byte absolute static-instruction ceiling, the tightest constraint this stage hit): the shape's `kernel.md` growth estimate (181→~196 lines) was not achievable — `pilot-product-slice/ideation`'s pre-change headroom was only ~150 bytes, so `kernel.md`'s unification prose was cut to one line ("Grade removals too.") and the vocabulary explanation was moved entirely into `reverse-recovery-audit.md` (not ceiling-bound), which now carries both tiers' full statement and cross-references `kernel.md`'s unchanged four reasons verbatim. `pilot-product-slice/shape.md`'s planned `semantics_unchanged` documentation sentence was dropped for the same reason (byte-ceiling; `production/shape.md` kept it — it had headroom); `choose-work-profile/SKILL.md` (not ceiling-bound) carries the field's documentation for both profiles instead. Post-change headroom, worst case: `pilot-product-slice/ideation` 128 bytes (verified this clears the largest existing ablation mutant's kernel.md growth, 69 bytes, with margin).
- Tests run clean in this worktree: `python3 scripts/kc-dev-flow-contract-test.py` (PASS), `python3 kc-dev-flow/scripts/profile-contract-loader.test.py` (PASS), `python3 kc-dev-flow/scripts/profile-spacedock-route.test.py` (PASS), `python3 scripts/kc-dev-flow-minimal-stack-ablation.test.py` (PASS, 63 mutants all REJECTED, up from the delivery-base's 60).

### Summary

`kernel.md` and `reverse-recovery-audit.md` now share one `need` vocabulary (`REQUIRED`/`NO_OBSERVED_CONSUMER`/`UNKNOWN`) with an identical worked case, one `disproof_hook` primitive at two declared tiers, and removal grading by the same predicate as retention. `profile-contract-loader.py` gained `semantics_unchanged` (required at Pilot/Production `ideation`) and the two equivalence-evidence scalars (required at `validation` when `true`), both fail-closed via `ContractError`, exercised by three new loader-test cases and two AC-7 backward-compat cases. The dominant implementation cost was the pre-existing 40000-byte static-instruction ceiling on `kernel.md`+base+stage, which forced most of the vocabulary prose out of `kernel.md` into the conditionally-loaded `reverse-recovery-audit.md`; this is a smaller `kernel.md` footprint than the shape estimated, not a scope cut — AC-1/AC-2/AC-3 are each satisfied by a verbatim, ablation-guarded assertion. All four test suites pass; diff stays under all three declared stops.
