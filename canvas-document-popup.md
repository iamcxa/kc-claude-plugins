...
- DONE: Exact delivered artifact recorded: the files changed and nothing beyond the approved scope
  Commit `6f4ec0e3805c7552f8b8377f7f0481f4ec9316ec` on `spacedock-ensign/canvas-document-popup`, 13 files: `lib/repo-doc.{mjs,test.mjs,d.mts}`, `server/repo-doc.ts`, `server/canvas-server.ts`, `server/client/{App.tsx,DocPopup.tsx,doc-popup.css}`, `vite.config.mts`, `package.json`+lock, `scripts/popup-browser-check.mjs`, `skills/kc-journey-map/references/canvas.md`. `vite.share.config.mts` and `human-led-review.md` confirmed untouched (`git status --porcelain` empty for both).
- DONE: The popup mounts outside tldraw's click-through slot; the Close control and Open on GitHub each receive a real pointer click in the delivered code, asserted by effect
  `App.tsx` portals `DocPopup` to `document.body` as a sibling of `<Tldraw>`, not inside `components.InFrontOfTheCanvas`. `scripts/popup-browser-check.mjs` clicks both controls via `agent-browser click` (a CDP pointer dispatch that refuses a covered target) and asserts the dialog is removed / a new tab opens targeting the source URL, not just that a key or DOM event fired.
- DONE: elementFromPoint at each interactive control's bounding-box centre resolves to that control or a descendant — the check that would have caught the defect the Captain found
  `hitTestable()` in the check script runs this exact assertion for Close and Open-on-GitHub in the ready state, the loading state, and all three unavailable states. The falsifier: moving `DocPopup` back inside `InFrontOfTheCanvas` reintroduces `.tl-canvas__in-front { pointer-events: none }` and this check fails.
- DONE: Escape re-verified in the delivered code, not assumed preserved from the preview
  Re-opened and closed via `Escape` independently of the Close-button check, asserting dialog removal and focus restoration, against the committed `DocPopup.tsx`.
- DONE: AC-1 through AC-5 each exercised the way a person uses them; browser evidence separated from HTTP or parser results
  AC-1 (no new tab, local), AC-2 (Mermaid autonumber `1..4`, embedded `<script>` did not execute), AC-3 (Close/Escape/GitHub-link real clicks, focus/selection/camera/full store-snapshot equality), AC-4 (loading via a delayed-fetch shim, bad-ref, bad-path, unmapped-repo, missing-heading — each with Open on GitHub present and hit-testable) all exercised via `agent-browser` against a real running canvas+Vite pair. AC-5 exercised by exporting the candidate's store snapshot and importing it into a `git archive d701df3d` (last commit before this viewer) bare checkout with no popup code, then clicking the plain `<a target=_blank>` there. 31 PASS, 0 FAIL — `/tmp` run logs not retained; re-run via `npm run check:popup-browser`.
- SKIPPED: AC-1's shared-board-origin half attempted and its result reported, including if it could not be run here
  Attempted via `agent-browser open --headers '{"Host":"sharedorigin.test"}'` against the real Vite dev server; the navigation hung and never completed — Host is a reserved header CDP will not rewrite for a real network request. Substituted an HTTP-layer check (`curl --resolve`): the configured host gets 200, an unlisted host gets 403, confirming `JOURNEY_ALLOWED_HOSTS` is wired, but this is not AC-1 proof by the entity's own verification clause (browser evidence required). A real browser check needs a DNS or hosts-file mapping this environment can't make without root.
- DONE: JOURNEY_DOC_REPOS startup line added to references/canvas.md's existing startup step, and no other retained-document edit
  One sentence added to the existing "Give each test service its own port..." paragraph in `references/canvas.md`, naming `git -C <launch dir> remote get-url origin` as the value's source per the Captain's ruling. `human-led-review.md` untouched.
- DONE: Tests cover loading/rendering integration and focus restoration, per the issue's own requirement
  `lib/repo-doc.test.mjs`: 11 unit tests (in CI via the existing `node --test lib/*.test.mjs` glob); the discriminating one is `candidateRefSplits('feature/foo/docs/x.md')` returning the longest-ref-first order — reversing that loop's direction fails it. `scripts/popup-browser-check.mjs`: 31 browser checks covering loading render, Markdown/Mermaid rendering, and focus restoration after Close and after Escape, each via a real pointer click or key press. Requires `agent-browser` on PATH (same requirement as the existing `scripts/sequence-smoke.mjs`); not wired into CI — `kc-journey-map-tests.yml` runs no browser, and adding one needs `agent-browser` on the runner plus unmeasured CI minutes. Captain's call, not made here.
- DONE: Pilot seams named: what real persistence and external seams were exercised, and what was only reasoned about
  Exercised: a real `git` subprocess against a real checkout (`execFile('git', ...)`, no mocking); the real Fastify `/repo-doc` route through the real Vite dev proxy; real CDP pointer clicks; a real export→import round trip into a second, independently-built checkout. Reasoned only: `npm run share` has no `/repo-doc` proxy entry (deliberately, matching `/tunnel` and `/save`'s existing operator-only treatment), so the popup there degrades to the `unavailable` state — Vite's 404 HTML makes `r.json()` throw into the existing `.catch`, not a special case; a genuinely private GitHub repo, which collapses into the unmapped-repo state under D1 and is accepted by the entity as such, not re-tested here.
- DONE: The Captain's canvas on port 3742 untouched; material limits recorded
  PID 6259 on 3742 and the ideation preview's PIDs (22811 on 5959, 23433 on 3799) identical before and after every run in this stage. Material limits: AC-1 shared-board-origin not verified by a browser run here (see above); `npm run share` has no popup (reasoned, not tested); a genuinely private-repo denial collapses into the unmapped state under D1 (accepted by the entity). Doctor's port-3737 failure is a pre-existing unrelated process on this machine (PID 98247), not caused by this change.

### Summary

Delivered the canvas-document-popup viewer: a Fastify `/repo-doc` route resolving a canonical GitHub blob link against a local git checkout, a Vite proxy entry, and a client-side popup that intercepts a chapter-link click and portals a dialog to `document.body` (not tldraw's click-through `InFrontOfTheCanvas` slot). Real-browser evidence via `agent-browser` (31 checks, all passing) covers AC-1 through AC-5, the Close/Escape/GitHub-link controls with elementFromPoint hit-testing, and a full canvas-store-snapshot equality check across open/close. One implementation-time defect found and fixed: an added `close`-event listener (beyond what the ideation preview validated) closed the popup immediately on first open under React StrictMode's mount-cleanup-mount cycle, because Chromium emits a spurious trusted `close` event after the second `showModal()`; removed, reverting to the preview-validated `onCancel`-only design — residual exposure is none, since a non-cancelable close-watcher `cancel` still reaches `onCancel`. AC-1's shared-board-origin half could not be verified by a browser run in this environment (Host header is CDP-unrewritable); an HTTP-layer substitute is reported in its place. `references/canvas.md` gained one sentence for `JOURNEY_DOC_REPOS`; `human-led-review.md` and `vite.share.config.mts` are untouched.

## Stage Report: validation

- DONE: Independent verdict on commit 6f4ec0e3, with the primary evidence that decided it
  PASSED. `App.tsx` at 6f4ec0e3 portals `DocPopup` to `document.body` as a sibling of `<Tldraw>` (not inside `InFrontOfTheCanvas`) — I reproduced the exact scenario the Captain caught (real pointer click on Close, not Escape) on my own fresh server/Vite pair (ports 4001/4002, isolated fixture repo) and it closed correctly: dialog removed, focus restored to the trigger `<a>`, selection/camera byte-identical. `elementFromPoint` at the Close button's own bounding-box centre resolved to the button itself.
- DONE: AC-1's shared-board-origin half settled: either verified by a browser run against a non-localhost origin, or stated plainly why it cannot be run here and what that leaves unproven
  Settled: verified by a real browser run. The implementation report's "needs a DNS or hosts-file mapping this environment can't make without root" is contradicted — `agent-browser --args "--host-resolver-rules=MAP sharedorigin.test 127.0.0.1"` maps the fake hostname at the Chromium DNS layer (no `/etc/hosts`, no root), producing a genuine top-level navigation whose real HTTP `Host` header is `sharedorigin.test:4002`. Confirmed first against a bare Python HTTP server that echoed the received `Host` header. Then ran the full flow against the actual canvas + Vite (`JOURNEY_ALLOWED_HOSTS=sharedorigin.test`): navigation succeeded past `allowedHosts`, popup opened with no new tab (AC-1), Mermaid autonumber rendered `1..4` and the embedded script did not execute (AC-2), a real click on Close removed the dialog with focus/selection/camera preserved and Escape re-verified independently (AC-3), and the unmapped-repo (D1) state rendered "unavailable" with Open on GitHub present, hit-testable and clickable (AC-4 D1 case) — all over the non-localhost host. This closes the gap the implementation stage left open; it does not need to be re-flagged to the Captain.
- DONE: The delivered code checked against each AC by your own method, not by re-running the implementation's script alone
  AC-1/AC-2/AC-3/AC-4(D1) exercised via a self-built fixture repo, fresh ports (4001/4002, distinct from 3742/3799/3737/5858/5959), and hand-issued `agent-browser` commands (`click`, `mouse move/down/up`, `eval`) rather than `popup-browser-check.mjs` — see above. AC-4's other three branches (bad ref, bad path, missing-heading) and AC-5 (export/import) were not independently re-executed; confirmed by reading `server/repo-doc.ts` (ref/path resolution both fall through to the same `unavailable` result) and `node_modules/@tldraw/tlschema/dist-cjs/shapes/TLNoteShape.js:43` (`url: T.linkUrl`, a standard schema prop, not custom metadata, so ordinary tldraw export/import round-trips it) — reasoning-based confirmation, not a fresh browser run. `npm run typecheck` re-run clean independently. `lib/repo-doc.test.mjs`'s 11 tests re-run standalone (11/11 pass); the discriminating claim for `candidateRefSplits`'s longest-ref-first order was falsified by mutation — reversing the loop's iteration direction (`for (let i = 1; i <= segments.length - 1; i++)`) fails 2 of the 11 tests, restored after.
- DONE: Pilot seams: real persistence and external seams exercised named, separated from what was only reasoned about
  Exercised by me: a real `git init`/`commit` fixture checkout read via the server's real `execFile('git', ...)` (no mocking), the real Fastify `/repo-doc` route through the real Vite dev proxy, real CDP pointer clicks and mouse-coordinate clicks, and a real DNS-layer non-localhost navigation. Reasoned only, by me: AC-5's export/import round trip (schema reading only) and the three AC-4 branches I did not re-run (bad ref/bad path/missing-heading — same code path as the D1 case I did exercise). `npm run share`'s missing `/repo-doc` proxy: confirmed directly by reading `vite.share.config.mts` — only `/connect/` and `/uploads/` are proxied, matching the reported limit exactly. CI's browser exclusion: confirmed directly — `kc-journey-map-tests.yml`'s own header comment states "the browser is not exercised here"; no `agent-browser` install step exists in the workflow.
- DONE: Every claim the implementation report makes that you could not confirm is listed as unconfirmed
  Unconfirmed by me (not contradicted, just not independently re-run): AC-5's full export→import→open-in-bare-checkout round trip (confirmed only by schema reading, not by executing the round trip); AC-4's bad-ref, bad-path and missing-heading branches specifically (I exercised the structurally identical unmapped-repo branch, not these three); the StrictMode `close`-event defect described in the implementation Summary (I did not reproduce the React StrictMode double-mount race — took the report's account of it on trust); a genuinely private GitHub repository's behavior (both the entity and the implementation report accept its collapse into the unmapped state under D1 as a design decision, not a tested case, and I did not test it either).
- DONE: No candidate mutation: HEAD at 6f4ec0e3 unchanged, and the Captain's canvas on 3742 untouched
  `git log -1 --format='%H'` on the CODE worktree read `6f4ec0e3805c7552f8b8377f7f0481f4ec9316ec` before and after this stage; `git status --porcelain` empty throughout. PIDs on 3742 (6259), 3799 (23433) and 5959 (22811) identical before and after every run in this stage — confirmed via `lsof`. My own server/Vite pair ran on 4001/4002 only, in a `/tmp` fixture, and was killed and deleted at the end of this stage.
