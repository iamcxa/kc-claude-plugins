...
- FAILED: Prove the dependency installation’s required offline/no-lifecycle execution from the sealed packet.
  The end state is consistent with the frozen lock, but `dependency-install.stdout` contains only “added 261 packages”, stderr only a deprecation warning, and exit is 0. The exact cwd/argv, offline state and `--ignore-scripts` are asserted only in owner-authored provenance/contract; no execution receipt binds them. End-state hashes cannot distinguish the required command from an online or lifecycle-enabled install, and the prohibited install/test rerun was not used to manufacture that evidence.
...
- **Material — offline/no-lifecycle dependency provenance is not bound to execution.** Released/normal workflow: the future proof relies on a private dependency install before executing the supplied test. Observable harm: the packet cannot distinguish the authorized frozen-lock offline/no-script preparation from a networked or lifecycle-running install, so its prepared-target safety claim is unauditable. Protected boundary: `captain-ruling[2026-09-21]` requires the checked-in frozen lockfile without upgrades or lifecycle scripts, and this validation assignment requires offline provenance. Trigger: generic npm stdout/stderr/exit and exact end-state hashes exist, but no captured cwd/argv or independent offline/ignore-scripts receipt exists. A packet evidence repair is required; no reinstall is authorized here.
...
The sealed packet is hash-consistent and its existing child custody/finalization code plus dependency end state are real, but it is not safe or concretely executable for a paid proof. Supplied PR tests inherit the trusted host OAuth variable, the atomic claim/total supervisor/delivery receipt has no sealed operation, and the required offline/no-lifecycle install is not execution-bound. Validation therefore returns **REJECTED** without product mutation, dependency/test rerun or model call.
