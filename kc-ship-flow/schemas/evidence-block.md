...
--what-changed-file <file>) [--lead <file>] [--evidence-file <file>] [--fixes <issue-id> ...]` — the
two scripts that actually parse a worker's Evidence block. Between them they read `CANDIDATE_SHA`,
`BASE_SHA`, `BRANCH`, `FILES`, `TESTS`, `WITHOUT_IT_COMMAND`, `WITHOUT_IT_REMOVED_VARIANT`,
`WITHOUT_IT_OBSERVED`, and `SELF_CHECK`. `accept-evidence.sh` requires `CANDIDATE_SHA`, `BASE_SHA`,
`WITHOUT_IT_COMMAND`, and `WITHOUT_IT_REMOVED_VARIANT` to be non-empty (refusing an "incomplete
Evidence block" otherwise) and reads `BRANCH` and `FILES` without requiring them; `open-pr.sh`
separately requires `CANDIDATE_SHA`, `BASE_SHA`, `BRANCH`, `WITHOUT_IT_COMMAND`,
...
`FILES` — and reads `TESTS` (optional) only in `--what-changed-file` mode, to scan for a *recognized
suite or test runner* (`contract-test.py` — its bare form `contract-test` is admitted the same way —
`*.test.sh`, `*.test.py`, `*.test.mjs`, `npm test`, `npm run test:*`, `pytest`, `vitest`,
`node --test`) followed by an `N/N`/`exit 0` token — a bare data or fixture path is never a suite,
however path-like, and an AC line is never counted. A runner-shaped token that names no recognized
suite prints a one-line notice on stderr (`evidence: no suite token matched; pass --evidence-file`)
rather than silently omitting the section. `--lead <file>` and `--evidence-file <file>` supply the
lead or the Evidence bullets verbatim when extraction cannot (a first sentence over 25 words with no
clause boundary, including an empty or whitespace-only source paragraph; no recognized suite token);
`--fixes <issue-id>` (repeatable) names another receipt issue this same PR also fixes, one more
`Fixes` line each, refusing an id absent from the receipt; see `references/stations/open-pr.md`.
