...
--what-changed-file <file>) [--lead <file>] [--evidence-file <file>]` — the two scripts that actually
parse a worker's Evidence block. Between them they read `CANDIDATE_SHA`, `BASE_SHA`, `BRANCH`,
`FILES`, `TESTS`, `WITHOUT_IT_COMMAND`, `WITHOUT_IT_REMOVED_VARIANT`, `WITHOUT_IT_OBSERVED`, and
`SELF_CHECK`. `accept-evidence.sh` requires `CANDIDATE_SHA`, `BASE_SHA`, `WITHOUT_IT_COMMAND`, and
...
`open-pr.sh` does not read `FILES` — its PR body's `## What changed` comes from `--entity-path`'s dev
entity or `--what-changed-file`, never from `FILES` — and reads `TESTS` (optional) only in
`--what-changed-file` mode, to scan for a *recognized suite or test runner* (`contract-test.py`,
...
however path-like, and an AC line is never counted. `--lead <file>` and `--evidence-file <file>`
supply the lead or the Evidence bullets verbatim when extraction cannot (a first sentence over 25
words with no clause boundary; no recognized suite token); see `references/stations/open-pr.md`.
