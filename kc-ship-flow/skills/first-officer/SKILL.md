...
```bash
python3 "$SHIP_SCRIPTS/local-profile-check.py" docs/ship/README.md
```
...
Write and check each stage's pin with `$SHIP_SCRIPTS/pin.py write --station <name>` /
...
```bash
$SHIP_SCRIPTS/ci-covers.sh <repo-root> <package-path> <check-name>
```
