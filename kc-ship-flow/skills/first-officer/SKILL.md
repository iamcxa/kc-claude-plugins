...
3. `reviewed` — `kc-ship-flow/scripts/open-pr.sh <evidence-file> <batch-dir>`, then
   `kc-ship-flow/scripts/disposition.py`. When the diff touches a dependency manifest or lockfile,
   dispatch the supply-chain lane (`kc-pr-flow:tob-supply-chain-checker` or the profile's equivalent)
   and write its findings before calling `disposition.py` — passed a bundle directory, it refuses
   (exit 2, `supply-chain findings required`) when that path is absent.
