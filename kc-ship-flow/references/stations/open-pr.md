...
[--evidence-file <file>] [--fixes <issue-id> ...]`
...
- **Lead** (no heading, never a fragment): the source paragraph's `## The problem` first sentence,
  skipping a leading sentence that is only a quoted attribution (`Speaker, date: 「...」.` — including
  one wrapped in backticks). An empty or whitespace-only source paragraph is treated the same as a
  missing lead, never emitted as an empty line. A sentence of `<=25` words is used as-is. A longer one
  is closed at the longest clause boundary (`,` `;` `—` `:`) within the first 25 words,
  period-terminated — never cut mid-clause. With no such boundary (or an empty/whitespace-only
  paragraph), `--lead <file>` (one sentence) supplies the lead; without it, the run refuses (exit 2,
  `lead required`).
...
- **`## Evidence`**: with `--entity-path`, one bullet per recognized-suite `N/N` or `exit 0` token
  ...
  names a recognized suite or test runner (`contract-test.py` — its bare form `contract-test` is
  admitted the same way — `*.test.sh`, `*.test.py`, `*.test.mjs`, `npm test`, `npm run test:*`,
  `pytest`, `vitest`, `node --test`); a bare data or fixture path is never a suite, however path-like.
  Omitted when no recognized token is found, unless `--evidence-file <file>` supplies bullets verbatim
  (one per line). When `TESTS` carries a runner-shaped token (`N/N` or `exit 0`) that names no
  recognized suite and no `--evidence-file` rescues it, a notice prints on stderr (`evidence: no suite
  token matched; pass --evidence-file`) — the section is still omitted and the run still exits 0.
- **`---` + audit link + `Fixes`**: unchanged by either mode — the issue matched to the Evidence
  block's `BRANCH` in the batch receipt (see below), always, plus one more `Fixes` line per
  `--fixes <issue-id>` naming another issue present in the same receipt (a one-PR delivery closing
  more than one issue) — an id absent from the receipt refuses (exit 2).
...
`BRANCH` matching zero or more than one issue, a `--fixes <issue-id>` absent from the receipt, an
untracked `batch-dir/README.md`); `what-changed required` (exit 2) when neither `--entity-path` nor
`--what-changed-file` is given; and `lead required` (exit 2) when the first sentence exceeds 25 words
with no clause boundary and no `--lead` override, including an empty or whitespace-only source
paragraph.
