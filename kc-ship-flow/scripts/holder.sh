...
set -euo pipefail
cmd=${1:-}; state=${2:-}; holder=${3:-}; rec="$state/_holder.json"
...
sync_in() {
  ...
  read remote branch < <(state_remote_branch)
  git -C "$state" fetch -q "$remote" "$branch" || die "fetch failed; refusing to act on local state" 6
...
write_rec() { python3 -c "import json,sys,time;json.dump({'writer':int(sys.argv[1]),'holder':sys.argv[2],'at':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime())},open('$rec','w'),indent=1)" "$1" "$2"; git -C "$state" add _holder.json; git -C "$state" -c user.name=holder -c user.email=holder@local commit -q -m "holder: writer $1 -> $2"; read remote branch < <(state_remote_branch); git -C "$state" push -q "$remote" HEAD:"$branch" || die "push rejected (someone else moved the writer); refusing" 6; }
