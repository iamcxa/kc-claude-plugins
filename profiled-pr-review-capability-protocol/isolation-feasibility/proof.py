...
def exercise(core, isolated, mode, guarded=False):
    base = ROOT / (('guarded' if guarded else 'isolated' if isolated else 'control') + '-' + mode)
    base.mkdir()
    ...
    command = ['/bin/bash', '-c', 'exec "$1" "$2" --child "$3" --mode "$4"', 'proof', sys.executable, str(ROOT / 'proof.py'), str(base), mode]
    if isolated:
    ...
    def operation():
        _, elapsed = core.pilot_invoke(args, current, command, base / 'work',
            {**os.environ, 'TMPDIR': str(base / 'temp'), 'ANTHROPIC_API_KEY': 'SYNTHETIC-NOT-A-CREDENTIAL', 'PYTHONDONTWRITEBYTECODE': '1'})
