...
#### Captain scope disposition for implementation cycle 12 — 2026-09-06

Captain replied `批准` to: `是否批准只將專項測試上限調至 1,903 行，其餘上限與隔離缺陷的阻擋維持不變？` The complete focused limit is now **1,903**, replacing **1,800** for this task only; the **6,600 total changed lines / 20 product files** limits remain unchanged. The denominator still includes schema, catalog, corpus, protocol tests, runtime tests and all ablation test changes, including new regressions. No tests are excluded or weakened by this decision.

The cycle-12 local report remains historical and unchanged: **6,443 / 20 / 1,903** failed its then-current 1,800 focused limit. Those recorded counts fit the newly accepted limit, but this is a scope decision, not a new test run, completed candidate or validation PASS. The referenced product remains at HEAD `9bb526170156a44cff90e2a2fab9eeab081e0eb1` plus the uncommitted six-path patch SHA-256 `fa9108f8bb0646139be7a8e222daeaa1a990d732706af6702d835931c779ee1e` preserved in `local-correction/`.

After asking to temporarily ignore unrelated shared-state changes, Captain replied `確認` to the bounded proposal: save this 1,903-line approval once, preserve the other batches' 33 deletions, `只提交本任務紀錄，遇到遠端變動就停止，不自動重整提交，也不重啟產品修復或送審`. This authorizes only this FO-owned feedback-cycle append in `profiled-pr-review-capability-protocol/index.md` and its one-file state commit/publication, despite unrelated dirty paths. Before this write, local and remote state both equalled `b735d84c6e7993e8deac5587dc15608bbf941192`; the task and staging area were clean. The 33 unstaged deletions are confined to `batch-132baa8a24d6`, `batch-bcffef3632d8` and `batch-e7cb978711a0`; their complete diff SHA-256 was `295298e4d3f619c0cc41c487327ccfe9450cec1056aa93372a60611548ce01c5`.

Use an exact-path commit and ordinary push with revision/path readback, not the automatic rebase fallback of the normal state publisher. Stop if the frozen local or remote revision changes, another staged path appears, or the frozen unrelated tracked diff changes; do not stash, restore, delete, commit or otherwise recover another batch. Untracked evidence from another batch stays outside the exact-path commit and is left untouched. This state-only exception ends after publication/readback or a revision-check refusal and is not a standing dirty-holder exemption. The earlier product-stage pin exception remains expired; historical pin SHA-256 `8dbc54e264711a9d9da98a160726a3a9b4f846bed2d4e4b23196581fb5c67ea8` and held validation remain unchanged.

F6 measurement integrity remains BLOCKING; F14 sequence evidence, hosted cost, independent validation and real five-pair quality/speed evidence remain unresolved. No product edit/commit/push, worker dispatch, paid/model/cloud call, reviewer invocation, status/gate/pin change, PR, merge or release is authorized by this approval record.

The first recording attempt stopped before commit when shared state advanced from `b735d84c6e7993e8deac5587dc15608bbf941192` to `357662d8c67d9eb8ec394a9fe30229b7d6eb9947`. Captain then replied `繼續` to resuming the pending one-file record from a freshly checked revision. This resumes only its exact-path commit/publication under the same preservation and stop conditions; it does not reopen product work or create another scope change. The commit parent records the resumed state revision.
