...
def cleanup(cidfile, label):
    ...
    if before.returncode == 0:
        ...
        assert 'No such object' in before.stderr, before.stderr
    after = docker('container', 'inspect', cid)
    assert after.returncode != 0 and 'No such object' in after.stderr, after.stdout + after.stderr
    ...
    result.update(inspect_after_exit=after.returncode, exact_label_remaining=remaining.stdout.strip())
    return result


def exercise(mode):
    base = ROOT / ('container-' + mode)
    base.mkdir()
...
try:
    ...
    (ROOT / 'container-results.json').write_text(json.dumps({'elapsed_seconds': time.monotonic() - started, 'outcomes': outcomes}, indent=2) + '\n')
