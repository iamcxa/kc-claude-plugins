# batch-0a557023cc36 — ship-flow batch 2: dispatch dev entities; adopter portability

Plan receipt 0a557023cc36cc41 (session plan-flow-session-2026-09-09b-shipflow-batch2); approval go · 15 workspaces · concurrency 1 · repair 2 · Pilot (Captain 「approve」 2026-09-09).
Order: DEV-156 → DEV-151 → DEV-152 (#399, pre-built, enters at accepted) → DEV-153 → DEV-154 → DEV-155. Repo iamcxa/kc-claude-plugins, base main. Workers push branches; the FO opens PRs (open-pr.sh, body per pr-merge until DEV-151 lands). Captain merges by hand.

## Decision log

- 2026-09-09T14:05:31Z — batch opened.
- 2026-09-09 — DEV-156 accepted by the installed station at 04b896da; PR #401 opened by open-pr.sh, body per pr-merge template. Review dispatched. Premise gap recorded: dispatch build flag mode needs --checklist-file; the station writes a one-line procedural checklist.
- 2026-09-09 — DEV-152: the Captain merged #399 directly (main 39cb179b) before its accept/review stations ran here; recorded as merged with stations skipped (Captain's prerogative). DEV-152 Done.
- 2026-09-09 — #401 review: F1 (CI job lacks spacedock → contract-test red on every PR; the same trap as DEV-139's class, caught before merge this time), F2 absolute in a comment, F3 doc omits --checklist-file, F4 dead --effort path, F5 fixture-shaped workflow-dir default. Repair round 1 dispatched: install step copied from kc-dev-flow-release-gate.yml (pinned, sha-verified), fail closed without spacedock, bounded claim, doc fix, remove effort, require --workflow-dir. CI cost: one install step per run, not timed.
- 2026-09-09 — **DEV-156 verdict: MERGE #401 (head 87d13967).** F1–F5 fixed and FO-verified; CI required check passed with the new install step (cost: one pinned install per run, not timed). Captain merges by hand.
- 2026-09-09 — DEV-151 round 1 accepted by the station but refused by the FO on content: the body generated from an Evidence block alone is a file list, not the pr-merge template. Repair round 1 dispatched: the source is the dev entity (DEV-156 made it the dispatched unit) per the mod's extraction rules; FO-authored bullets as the fallback.
- 2026-09-09 — DEV-151 round 2: entity extraction proven (fixture + FO mutation), but a real run still emits a truncated lead and fixture-name Evidence bullets → repair round 2 (last within the approved two).
- 2026-09-09 — DEV-151 round 3 accepted (6cd2c881); PR #402 created by the installed open-pr.sh and its body generated by the candidate's own open-pr.sh (fallback path: FO bullets + evidence file), 117 words — the ticket's falsifier in practice. Review dispatched. 77 comment lines added in open-pr.sh: reviewer to judge.
- 2026-09-09 — #402 review: five findings, three of which touch the ticket's purpose (F2 multi-issue Fixes — #397's exact shape; F3 this batch's own `contract-test` token dropped silently; F1 empty lead passes). DEV-151 has used its two approved repair rounds. Captain to rule: a third round (re-sign repair 3 for this issue) or merge with a follow-up ticket.
- 2026-09-09 — DEV-153 accepted by the installed station at 2d54370c; PR #403 opened (open-pr.sh), body per template (FO-written until #402 lands). Review dispatched; the two edited files under docs/ship-flow/flows are a review question (non-goal DEV-120).
- 2026-09-09 — **DEV-153 verdict: MERGE #403 (head 2d54370c).** Review no findings; FO ACs green. Captain merges by hand.
- 2026-09-09 — DEV-154 accepted at c4bd54cb (pair restated to a grep so the removed leg is non-zero); PR #404 opened, body per template. Review dispatched. DEV-155 dispatched (last build of the batch).
- 2026-09-09 — #404 review: five findings (missed intent.md, guard scope, two stale claims, fallback depth). Repair round 1 dispatched.
- 2026-09-09 — DEV-155 accepted at fdc4d6ff (pair restated: base station as the removed variant); PR #405 opened, body per template. Review running.
- 2026-09-09 — **DEV-154 verdict: MERGE #404 (head a2a87725).** F1–F5 fixed; FO mutation of a station-doc line fails the new guard. Captain merges by hand.
- 2026-09-09 — #405 review: F1 (rename passes as add) and F2 (any path token satisfies the rule) confirmed; repair round 1 dispatched (true-add set with rename detection; executed path only; three contract cases).
- 2026-09-10 — **DEV-155 verdict: MERGE #405 (head 7d0ac8cc).** F1/F2 fixed and proven by FO mutations (note: removing `-M` alone is a no-op because git's diff.renames defaults on; `--no-renames` is the real mutation). Captain merges by hand. All five builds are at the merge station: #401, #403, #404, #405 mergeable; #402 awaits the Captain's ruling.
- 2026-09-10 — Captain merged #401 (DEV-156 → bde4b49a) and #403 (DEV-153 → 6b408ac1). #404 and #402 now conflict with main (moved_base); a repair worker merges main into #404 (SKILL.md, e2e-gate.md). DEV-156, DEV-153 Done.
- 2026-09-10 — #404 moved_base resolved (merge commit 07608ef2); FO verified contract-test and the prefix guard; **MERGE #404** stands. 
- 2026-09-10 — Captain re-signed a third repair round for DEV-151 (「走第三輪」): approval repair_rounds 2 → 3 for this issue only; workspaces unchanged (the same branch, a fresh worker). Round 3 also carries #402's moved_base merge. Five findings F1–F5 in review/findings-402.json.
