...
- DONE: Fix 1 — ARCHITECTURE.md "Repository layout" paragraph reworded so each named subdirectory claim is checkable per the actual set of plugins that ship it.
  `ls` on all 6 plugin dirs (`e2e-pipeline`, `kc-plugin-forge`, `kc-nightwatch`, `kc-hyperfocus`, `kc-team-ops`, `kc-pr-flow`) confirmed: `skills/` present in all 6 (universal), `agents/` present in 5/6 (absent only in `kc-plugin-forge`, which has `.claude-plugin/`, `.codex-plugin/`, `docs/`, `hooks/`, `reference/`, `skills/`, `smoke-tests/` at top level), `hooks/` present in 4/6 (matches the existing "Hooks and scripts surfaces" section, unchanged). Reworded the sentence in ARCHITECTURE.md's "Repository layout" section (worktree `.worktrees/spacedock-ensign-product-architecture-baseline/ARCHITECTURE.md`) from "each with its own `skills/`, `agents/`, and (where applicable) `hooks/` directories" to "each with its own `skills/` directory; five of the six also ship `agents/` (all but `kc-plugin-forge`), and (where applicable) `hooks/` directories" — universal claim for `skills/`, named-exception claim for `agents/`, unchanged qualified claim for `hooks/`.
- DONE: Fix 2 — ARCHITECTURE.md "Versioning scheme" paragraph's `.claude-plugin/plugin.json` / `.codex-plugin/plugin.json` references gain the `<plugin>/` prefix so they read as per-plugin paths, not repo-root paths.
  Confirmed no root-level `.claude-plugin/plugin.json` exists (root `.claude-plugin/` contains only `marketplace.json`; verified `find . -maxdepth 1 -name .claude-plugin` plus `ls .claude-plugin/`). Edited both citations in the "Versioning scheme" section to `<plugin>/.claude-plugin/plugin.json` and `<plugin>/.codex-plugin/plugin.json`. Left the "Marketplace publish flow" section's `.claude-plugin/marketplace.json` reference unprefixed — that one genuinely is repo-root (confirmed present at `.claude-plugin/marketplace.json`), so no change was needed there.
- DONE: Re-verify AC-3 still holds after the edit, and re-run the heading spot-check.
  `git diff --stat origin/main -- PRODUCT.md ARCHITECTURE.md` → "2 files changed, 86 insertions(+)", 0 deletions; `git diff origin/main -- PRODUCT.md ARCHITECTURE.md | grep '^-' | grep -v '^---'` → empty (no deletion lines). Byte-for-byte re-check of both kc-pr-flow sections against `origin/main` (`diff <(git show origin/main:PRODUCT.md | sed -n '5,$p') <(sed -n '41,$p' PRODUCT.md)` and the ARCHITECTURE.md equivalent, origin line 5 vs current line 55) both report identical — the two kc-pr-flow sections remain untouched. Heading spot-check: `grep -n "^## " PRODUCT.md ARCHITECTURE.md` shows all 6 plugin catalog entries in PRODUCT.md plus the kc-pr-flow entry, and ARCHITECTURE.md's 4 named surfaces (Repository layout, Marketplace publish flow, Hooks and scripts surfaces, Versioning scheme) plus the kc-pr-flow entry — unchanged from cycle 0.
  Committed as `4e47d02` on `spacedock-ensign/product-architecture-baseline`, on top of `f28bdd2` (new commit, not amended). 7 insertions / 6 deletions confined to ARCHITECTURE.md only — PRODUCT.md untouched in this cycle.

### Summary

Cycle-1 rework: applied both confirmed fixes from the FO gate override (ARCHITECTURE.md:9's `agents/` universality claim and ARCHITECTURE.md:47's unprefixed `plugin.json` path), re-verifying against the actual tree before editing rather than trusting the prior pass. Both fixes confined to ARCHITECTURE.md's "Repository layout" and "Versioning scheme" paragraphs — the paragraph the ideation stage itself flagged as lower-priority orientation prose, not a diffable contract surface. Re-ran AC-3 and the heading spot-check post-edit: both still pass, PRODUCT.md unchanged, kc-pr-flow sections in both files still byte-identical to `origin/main`. New commit `4e47d02` on top of the validated reference point `f28bdd2`.

## Stage Report: validation (cycle 1)

- DONE: Cycle-1 re-review, narrow scope — first-hand verify the two rework fixes in commit 4e47d02 against the actual tree: "Repository layout" claims hold for the real 6-plugin set; "Versioning scheme"/"Marketplace publish flow" path prefixing is accurate; confirm 4e47d02 contains only these edits (ARCHITECTURE.md only, ~7/6) on top of f28bdd2.
  `git show --stat 4e47d02` → "ARCHITECTURE.md | 13 +++++++------", "1 file changed, 7 insertions(+), 6 deletions(-)" — matches exactly, no other file touched, sits on top of `f28bdd2` per `git log --oneline -5` (4e47d02 → f28bdd2 → 755cf96 origin/main). Per-plugin `ls -d <plugin>/{skills,agents,hooks}` across all 6 plugin dirs: `skills/` present in 6/6 (universal, as claimed); `agents/` present in 5/6, absent only in `kc-plugin-forge` (matches the reworded sentence's named exception verbatim: "five of the six also ship `agents/` (all but `kc-plugin-forge`)"); `hooks/` present in 4/6 (`e2e-pipeline`, `kc-plugin-forge`, `kc-hyperfocus`, `kc-pr-flow`), matching the unchanged qualified "(where applicable)" wording and the existing "Hooks and scripts surfaces" section. Versioning scheme: `test -f <plugin>/.claude-plugin/plugin.json` true for all 6 plugins, and root `.claude-plugin/` contains only `marketplace.json` (`ls -la .claude-plugin/`, `find . -maxdepth 2 -name plugin.json` finds only the 6 per-plugin paths, zero root-level hits) — so the `<plugin>/`-prefixed citations are accurate and the unprefixed `.claude-plugin/marketplace.json` in "Marketplace publish flow" is correctly left unprefixed (genuinely repo-root). `test -f <plugin>/.codex-plugin/plugin.json` true only for `e2e-pipeline`, `kc-plugin-forge`, `kc-pr-flow` — matches the "(only ... ship a Codex manifest)" parenthetical unchanged by this fix. No claim in the fix diff names a path or behavior absent from the repo — AC-1's falsified-by clause is not tripped by this cycle's edit.
- DONE: Re-confirm AC-3 on the new HEAD — `git diff origin/main -- PRODUCT.md ARCHITECTURE.md` shows insertions only (0 deletions) and both kc-pr-flow section bodies remain byte-identical to `origin/main`.
  `git diff --stat origin/main -- PRODUCT.md ARCHITECTURE.md` → "ARCHITECTURE.md | 50 ++...", "PRODUCT.md | 36 ++...", "2 files changed, 86 insertions(+)" — 0 deletions reported. `git diff origin/main -- PRODUCT.md ARCHITECTURE.md | grep '^-' | grep -v '^---'` produced zero matched lines (grep exit 1), confirming no deletion lines anywhere in the diff, not just in the stat summary. Byte-for-byte re-check: `diff <(git show origin/main:PRODUCT.md | sed -n '5,$p') <(sed -n '41,$p' PRODUCT.md)` → empty/exit 0; `diff <(git show origin/main:ARCHITECTURE.md | sed -n '5,$p') <(sed -n '55,$p' ARCHITECTURE.md)` → empty/exit 0 (heading line numbers reconfirmed via `grep -n "^## kc-pr-flow"` in both origin and current trees before diffing). Both existing kc-pr-flow sections remain untouched.
- DONE: Verdict PASS or REJECTED with actual command output; assess any new finding against AC-1's falsified-by clause first; do not re-litigate cycle-0's already-accepted ground.
  No new finding surfaced in this narrow re-review — every claim touched by the fix diff was independently re-derived from the live tree (not trusted from the implementer's report) and matched. Cycle-0's broader sweep (cross-model gate, accuracy sweep, adversarial residuals) is out of scope for this cycle per the dispatch's explicit instruction and was not re-run. **Verdict: PASS.**

### Summary

Narrow cycle-1 re-review of commit `4e47d02` (the two-fix rework from the FO gate override): confirmed the commit is ARCHITECTURE.md-only, 7 insertions/6 deletions, sitting on top of the already-validated `f28bdd2`. Independently re-derived every claim the fix diff touches from the live tree — `skills/` universal (6/6), `agents/` 5/6 with `kc-plugin-forge` the sole named exception, `hooks/` qualified (4/6, unchanged); per-plugin `.claude-plugin/plugin.json` exists for all 6 while no root-level `plugin.json` exists, `.codex-plugin/plugin.json` exists only for the 3 named plugins, and the unprefixed `.claude-plugin/marketplace.json` citation in "Marketplace publish flow" is correctly left unprefixed since that file genuinely lives at repo root. Re-confirmed AC-3 on the new HEAD: 86 insertions / 0 deletions vs `origin/main` across both files, zero deletion lines in the raw diff, and both kc-pr-flow section bodies byte-identical to `origin/main`. No new finding, nothing re-litigated from cycle-0's already-accepted ground. Verdict: PASS.
