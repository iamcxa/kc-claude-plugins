...
Run: python3 kc-dev-flow/scripts/profile-contract-loader.py --work-item .context/DEV-89-work-item.md --local-profile docs/dev/README.md --format text
Follow the shared core, POC base, and build contract. Standalone item; the Brief is the planning authority.

## Step 3 — the raw requirement (station input)

```
Three ship-flow contract changes returned to planning by DEV-67 (Codex round 3):
(a) dispatch a higher layer only after the lower layer is fully verified (this replaces an earlier "freeze the lower layer" rule that proved self-contradictory);
(b) a worker's without-it command runs in an isolated environment (temporary HOME, no agent, no network), not merely with credential variables unset;
(c) security, data-loss, and compatibility findings outside the Brief block the candidate; only general improvements are scoped out.
Context: ship-flow is the workflow that takes a plan receipt (a Linear Project with lint-passing Issues) and dispatches one cloud worker per Issue, verifies each candidate at a pinned SHA (without-it, contract test, Codex review), and hands the Captain one UAT document. These three sentences are currently prose in a Spacedock entity's change list; nothing enforces them. The user of ship-flow is the Captain (Kent), who wants to plan carefully, hand off dispatch and verification, and only do final UAT.
```

## Step 4 — run the dialectic twice

Install the borrowed skills for the first run: claude plugin marketplace add deanpeters/Product-Manager-Skills && claude plugin install problem-statement@pm-skills epic-hypothesis@pm-skills user-story-splitting@pm-skills -y. Then invoke each via the Skill tool by name. Do NOT copy any pm-skills text into the repository; it is CC BY-NC-SA and this repo is public.

Run A (borrowed): station 1 problem-statement on the raw requirement -> draft `## The problem`; station 2 is the FO's: write four placeholders `USER_VALUE: <FO fills>` and `PROFILE: <FO fills>` and continue with your own best one-line user value as a stand-in, clearly marked STAND-IN; station 3 epic-hypothesis -> draft `## Goal` plus a `Falsifier:` line; station 4 user-story-splitting -> a list of Issues (title, one-paragraph body in the Development Brief shape: The problem / Goal / Non-goals / Acceptance criteria with AC-N / Route-back conditions) and a blockedBy list. For each station record: consumed input (which text), produced output (verbatim), which Brief field it filled, and whether the FO would have to rewrite it (count sentences you had to change to make it fit).

Run B (fallback): uninstall the three pm-skills (`claude plugin uninstall <name>@pm-skills` each) and confirm with `claude plugin list`. Write `kc-plan-flow/references/dialectic.md` (create the directory) with four stations; each station names the borrowed skill it prefers when installed, the Brief field it fills, and a fallback of at most three questions in your own words that a First Officer asks when the skill is absent. Then run the four stations again using ONLY those fallback questions, producing the same artifacts as Run A.

Write both runs to `.context/dev-89/run-A.md` and `.context/dev-89/run-B.md`, and a comparison `.context/dev-89/compare.md` (Issue count, titles, blockedBy edges, and the per-field verbatim-vs-rewritten table for both). Paste the three files' full contents into your Evidence block under RUN_A, RUN_B, COMPARE so the FO can read them from the transcript (the .context files are gitignored and unreachable).

## Step 5 — commit and push only dialectic.md

- git fetch origin main && git checkout -b feature/dev-89-poc-plan-flow-dialectic-borrowed-pm-skills-versus-kernel 216a29bc2cbaec19ca98e519025da4483b6be491
- Commit `kc-plan-flow/references/dialectic.md` only. Run python3 scripts/kc-dev-flow-contract-test.py in the background, poll, and record the exit code.
- One commit: docs(kc-plan-flow): draft dialectic stations with borrowed skills and fallback questions
- git push origin HEAD:refs/heads/feature/dev-89-poc-plan-flow-dialectic-borrowed-pm-skills-versus-kernel
- Do NOT open a pull request. Do NOT write to Linear.

## Step 6 — final reply: exactly one fenced block
...
MINUTES: station1=<n> station3=<n> station4=<n> runB_total=<n>
RUN_A:
<full contents of run-A.md>
RUN_B:
<full contents of run-B.md>
COMPARE:
<full contents of compare.md>
