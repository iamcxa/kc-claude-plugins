...
- Cycle 1: REVISE — ship FO independent verification at PR #457 head 5537e44c on macOS. Both
  test suites passed and no fixture copies, but `validate_admission_brief()` (AC-1's mechanism)
  had no live caller: `grep -rn -- --validate-admission kc-dev-flow/skills kc-dev-flow/references`
  found only the test file and one README table row; `continue-dev-flow`'s actual dispatch-time
  `profile-contract-loader.py` invocation omitted the flag. Replaying the validator on the three
  r4 briefs admitted today refused all three, proving the new bite/consumer refusal had no live
  path. Findings routed to `implementation`: (1) wire `--validate-admission` into the admission
  step workers actually execute; (2) prove it on the two closed r3 briefs plus one
  `DEVELOPMENT_BRIEF_SECTIONS`-shaped brief, exact refusal texts in Evidence; (3) disclose in
  Residuals that current backlog briefs lack the required section shape, headings listed
  verbatim. Corrected at candidate 874bbcef59933a4accd24e9d070d834b69e90c7c: wired into
  `continue-dev-flow`'s "Before dispatch" loader call at a Pilot/Production item's new admission;
  replay against `adopter-contract-test-ships-with-the-package.md` refused with the exact stderr
  line (independently re-verified by the FO in the ensign's worktree); `pr-merge-released-body-
  pin-per-mod-version.md` legitimately no-ops (its own `profile_choice_note` records it as
  `poc-exploration`, out of the check's profile scope); a reshaped brief passes. Residuals
  disclose 0/89 backlog entities currently satisfy the full admission shape. Round budget: one
  correction cycle, no escalation.

## Stage Report: implementation (cycle 2)

- DONE: Wire `--validate-admission` into the admission step workers actually execute (choose-work-profile pilot/production path, and continue-dev-flow where the first dispatch loads contracts); name the exact step in the PR body.
  `kc-dev-flow/skills/continue-dev-flow/SKILL.md`'s "Before dispatch" loader invocation is the wired call site: added `--validate-admission` to the exact `profile-contract-loader.py` command a Pilot/Production item's new admission runs (target `workflow_stage` is the profile's first route stage, `ideation`, with no prior stage pin), stopping before writing the stage pin or dispatching on a non-zero exit and quoting the `profile contract: ...` stderr line. `kc-dev-flow/skills/choose-work-profile/SKILL.md`'s admission-bar paragraph and `kc-dev-flow/README.md` were updated to name this caller instead of describing an unwired flag. Commit 874bbcef.
- DONE: Prove it on real briefs, not only fixtures: replay against the two closed r3 briefs and one r4 brief brought to the `DEVELOPMENT_BRIEF_SECTIONS`-required shape plus `bite:`/`consumer:` lines; put exact refusal texts in the PR's Evidence section.
  `adopter-contract-test-ships-with-the-package.md` (profile `pilot-product-slice`), replayed directly from `docs/dev/.spacedock-state/`: `profile-contract-loader.py --work-item <path> --validate-admission` exits 2, stderr `profile contract: admission brief must carry a concrete bite: line in its preamble`. `pr-merge-released-body-pin-per-mod-version.md` is profile `poc-exploration` (its own recorded `profile_choice_note` says it was kept POC because it does not meet the Pilot heading bar), so `validate_admission_brief()` correctly no-ops (line 493's profile guard) and exits 0 — this brief cannot demonstrate the refusal because it was never Pilot/Production; the sibling brief above is the one that does. A fixture built from this entity's own preamble/body (`## The problem` heading wrapping its narrative paragraph, `bite:`/`consumer:` lines added, `Acceptance criteria` bullets normalized from `*` to the required `- **AC-N**` marker) exits 0 with a `development_brief_sha256`. Exact texts recorded in PR #457's Evidence section.
- DONE: State in the PR's Residuals section that every brief in this repository's backlog lacks the `## The problem` section shape and will be refused at next admission; list the required headings verbatim.
  Scanned all 89 tracked files in `docs/dev/.spacedock-state/`: only 2 of 89 carry all five `DEVELOPMENT_BRIEF_SECTIONS` headings, and 0 of 89 carry both a concrete `bite:` and `consumer:` preamble line. Recorded in PR #457's Residuals section with the five required headings verbatim, in order, and the caveat that the check applies only at an item's new admission, not to already-admitted items.

### Summary

Wired `--validate-admission` into `continue-dev-flow`'s actual dispatch invocation at a Pilot/Production item's new admission (first working-stage dispatch out of `backlog`), replacing the prior state where the refusal existed but nothing called it. Replayed against both named r3 briefs: the `pilot-product-slice` one refuses with the exact bite: line; the `poc-exploration` one legitimately passes because the check is profile-scoped by design, not because the wiring missed it. A brief reshaped to the required section headings plus `bite:`/`consumer:` passes. Confirmed 0 of 89 backlog entities currently satisfy the full admission shape and disclosed that in the PR's Residuals with the verbatim heading list. Pushed candidate 874bbcef and updated PR #457's `Candidate:` line and body; work committed on `spacedock-ensign/admission-asks-who-was-bitten-and-who-will-run-it`.

## Stage Report: validation (cycle 2)

- DONE: Independently re-run `profile-contract-loader.test.py` and `kc-dev-flow-contract-test.py` fresh at candidate 874bbcef (worktree HEAD confirmed at `874bbcef59933a4accd24e9d070d834b69e90c7c`, matching PR #457's `headRefOid`), not reused from the implementation report.
  `python3 kc-dev-flow/scripts/profile-contract-loader.test.py` → 6/6 suites PASS incl. "admission bite/consumer preamble", exit 0. `python3 scripts/kc-dev-flow-contract-test.py` → PASS, exit 0.
- DONE: Independently replay `profile-contract-loader.py --work-item docs/dev/.spacedock-state/adopter-contract-test-ships-with-the-package.md --validate-admission` to confirm the exact refusal text (path is a flat `.md`, not a folder `index.md`, at this candidate).
  Exit 2, stderr `profile contract: admission brief must carry a concrete bite: line in its preamble` — byte-identical to the text quoted in PR #457's Evidence section.
- DONE: Confirm `continue-dev-flow/SKILL.md`'s Before-dispatch invocation actually carries `--validate-admission` at new admission.
  `kc-dev-flow/skills/continue-dev-flow/SKILL.md` lines 96-107: the new-admission code block adds `--validate-admission` to the same `--write-stage-pin` invocation, with the stop-before-pin/dispatch and stderr-quoting rule stated in the following paragraph (lines 109-115); a subsequent-stage dispatch correctly omits the flag (loader no-ops for it).
- DONE: Confirm PR #457's body reflects the corrected candidate: updated `Candidate:` line, the wiring named as its own What-changed bullet, exact refusal texts in Evidence, and the Residuals disclosure (0/89 backlog entities satisfy the admission shape, required headings listed verbatim).
  `gh pr view 457`: `Candidate: 874bbcef59933a4accd24e9d070d834b69e90c7c` matches `headRefOid`; "Correction round" is its own bullet under What changed naming the `continue-dev-flow` wiring; Evidence quotes both replay commands' exact stderr/exit codes; Residuals states "only 2 of 89 ... 0 of 89 carry both a concrete `bite:` and `consumer:`" with the five `DEVELOPMENT_BRIEF_SECTIONS` headings in a verbatim, ordered code block.
- DONE: Re-confirm AC-1..AC-4 still hold at the new candidate (no kernel/references/kernel.md edit; no new receipt/harness/LOC gate).
  `git diff main...874bbcef --stat`: 12 files changed (README.md, 4 surface-map fixtures, profile-contract-loader.py/.test.py, surface-map-check.py, choose-work-profile/SKILL.md, continue-dev-flow/SKILL.md, run-batch/SKILL.md, kc-dev-flow-contract-test.py) — `references/kernel.md` absent from the diff (AC-4 kernel clause holds); no new JSON/YAML receipt schema, CI workflow, or LOC-counting file introduced; all additions are refusal conditions/wiring inside existing scripts and skills (AC-1/AC-2/AC-3/AC-4 hold).

### Summary

Independent re-verification at candidate 874bbcef (matching PR #457's head): both test suites pass fresh, the admission replay against the real `adopter-contract-test-ships-with-the-package.md` reproduces the exact cited refusal text, and `continue-dev-flow/SKILL.md` demonstrably wires `--validate-admission` into the new-admission dispatch it claims to. PR #457's body accurately reflects the corrected candidate across all four required elements (Candidate line, What-changed bullet, Evidence texts, Residuals disclosure). AC-1..AC-4 re-confirmed against the full `main...874bbcef` diff: no kernel edit, no new receipt/harness/LOC gate. No further correction needed.
