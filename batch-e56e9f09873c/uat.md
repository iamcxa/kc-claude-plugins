...
- Residual: Slack half not built: planning delta — DEV-113
...
- PR: https://github.com/iamcxa/kc-claude-plugins/pull/381 · candidate `9282343c43af` · base `8983967357fb` (main) · branch `feature/dev-107-ship-flow-close-the-close-receipt-refuses-an-undispositioned`
- Without-it (worker self-report): retained -> exit 0; removed -> exit 1; at BASE_SHA -> exit 1 · contract test (worker self-report) PASS
- FO accept station: 2026-09-06T02:59:11Z accept-evidence: ACCEPT
- Residual: review station result pending at draft time; rebased by the FO under moved_base
- How to verify: run `grep -q "defects_returned missing fix_ticket or accepted_residual" docs/plan-flow/schema/validate-receipt.py` (expect the retained exit above); apply `git checkout 8983967357fb457751fa0fbdbcd7965e41fede25 -- docs/plan-flow/schema/validate-receipt.py`; run it again (expect the removed exit above).
...
- Residual (DEV-106): Slack half not built: planning delta — DEV-113
- Residual (DEV-106): fixtures copy two real batch records into main \(retention: Captain's call\)
- Residual (DEV-107): review station result pending at draft time; rebased by the FO under moved_base
