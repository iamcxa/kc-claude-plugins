...
FO-verified at fba04c0f (fresh npm ci): full npm test exit 0 (unit 204, postgres 71, netlify-local 5, all stages); full-run self-check admitted, providerCalls 0, deploys 0. WITHOUT_IT: with ROLLBACK_MIGRATIONS set to 10 the session test "binds the admitted rollback, one candidate, supported status, runtime, and exact cleanup" fails (exit 1), the other 7 pass; restored.
