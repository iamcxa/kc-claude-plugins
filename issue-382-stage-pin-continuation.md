...
validation-worker-routing: "FO observed the live roster contains only /root; the prior completed validator is no longer addressable. Apply feedback-rejection-flow reviewer fallback once within the previously declared one-replacement allowance: one independent fresh-context validator in the same product worktree, exact candidate and unchanged verification assignment. No producer self-review, extra reviewer panel, extra correction round or additional provider observation. This is FO runtime routing under the existing worker-unavailable rule, not a new Captain scope decision."
---
