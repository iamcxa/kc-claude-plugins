...
work_profile:
  ...
  equivalence_instrument: 'DEV-51''s own Mutation replay table (cycles 1-2, corrected by the validation-bounce tier reclassification): apply the recorded mechanism patch to profile-contract-loader.py, engage-reconcile.py, poc-close-guard.py, or linear-admission.py, run the owning suite (or the full contract test where the rule surfaces only there), observe the new assertion redden naming the rule, git checkout -- restore, observe green; validation independently replayed 4 of 12 remaining-valid triples, one per mechanism, at candidate 370f02ba (re-verified after the bounce fix invalidated the prior 99a0c826 evidence).'
  equivalence_instrument_failure: 'At 370f02ba: engage-reconcile.py L164 return 1 if result["status"]=="delta" else 0 mutated to return 0 -> engage-reconcile.test.py exit 1, "membership delta returned 0"; poc-close-guard.py L120 heading "POC outcome" mutated to "POC outcome MUTATED" -> poc-close-guard.test.py exit 1, CloseError "work item must contain exactly one POC outcome MUTATED"; linear-admission.py L402 --expected-source prefixed "MUTATED-" -> kc-dev-flow-contract-test.py exit 1, "clean Linear admission failed: linear admission: planning comparator returned invalid output"; profile-contract-loader.py L697 if validate_admission mutated to if True (package README omits admission boundary site, still tier-1 after the bounce) -> profile-contract-loader.test.py exit 1, "profile contract: new admission cannot contain Acceptance evidence with canonical criteria" (the historical-fixture assertion the bounce fix reordered ahead of this items own default-mode check); all four reverted, reran green. Superseded reference: the prior receipt cited a validate_admission_brief() early-return "0"*64 mutation tied to kernel backlog exit bar is missing, a site the validation bounce restored to a phrase pin and is no longer tier-1; replaced with a still-valid triple on the same mechanism.'
  decision:
...
**Second finding, added at the 2026-09-03 re-validation**: the validation bounce (`## Validation bounce` below) found that `kc-dev-flow-contract-test.py --ablation-check` — the quick mode every ablation-suite document mutant was believed to be checked against — skips the `run()` block that shells out to the mechanism sub-suites (`profile-contract-loader.test.py`, `engage-reconcile.test.py`, `poc-close-guard.test.py`). Any tier-1 conversion whose sole evidence was "the mechanism suite already covers this" therefore had zero effective protection against the class of mutation (document-prose rewording) the deleted pin used to catch, until the full `kc-dev-flow-minimal-stack-ablation.test.py` was run and caught it. This is a systemic tooling gap in the contract-test harness itself, not specific to DEV-51's pins, and needs a follow-up item: no existing DEV issue names it (checked `DEV-11`, `DEV-12`, `DEV-46`, `DEV-59` via a Linear search on "ablation" — none fit). The Captain's authorization is needed to open a new follow-up issue for it; this validation stage does not create one unasked.
...
**Superseded by the validation bounce below**: this table's 18 rows predate
the bounce fix. Five pure tier-1 sites were subsequently restored to phrase
pins on a real ablation-survivor finding (`## Validation bounce`,
`### Corrected tier classification`), each with its own reason there
(`continuation is missing`, `continuation omits provider engage behavior`,
`adopter omits static Local Profile marker`, `kernel backlog exit bar is
missing`, `continuation omits doc trigger`). The current wording-only/tier-2
count under the Captain's route-back ruling is **18 + 5 = 23**, not 18; this
table is not re-duplicated with the 5 new rows to avoid stating the same
reason in two places — see the bounce section for each one's disposition.
...
## Stage Report: validation (2026-09-03, re-verify at 370f02ba)

Prior-cycle validation evidence at `99a0c826` is void (candidate changed: `origin/main` merged, then a bounce fix restored 5 misclassified pins). This section independently re-verifies the same three checklist items at the new exact candidate `370f02ba`, with stdin closed per the coordinator's exact commands.

- DONE: Run the exact contract-test and full-ablation commands with stdin closed; record exit codes, elapsed times, and rejected-mutant count. Replay at least four mutation triples from the current replay table, one per mechanism; confirm from `git diff origin/main...HEAD` that every removed pin has its replacement in the table and restored pins are byte-identical to origin/main's.
  Confirmed HEAD is `370f02ba`. `python3 scripts/kc-dev-flow-contract-test.py < /dev/null` → PASS, exit 0, 44.6s. `python3 scripts/kc-dev-flow-minimal-stack-ablation.test.py < /dev/null` → PASS, exit 0, 2m07s (first attempt hit this tool's 2-minute default and was retried with a longer timeout — the suite itself was never interrupted mid-run, just re-launched clean), **64 mutants REJECTED, 0 survivors** — matches the coordinator's own reference run (64/0, ~135s) within normal variance. Replayed 4 triples, one per mechanism, using sites the corrected tier table still lists as converted (not the now-restored ones): (1) `engage-reconcile.py` L164 mutated → `engage-reconcile.test.py` exit 1 "membership delta returned 0", restored, green — covers `continuation planning disambiguation omits`'s T1 half; (2) `poc-close-guard.py` L120 heading mutated → `poc-close-guard.test.py` exit 1 `CloseError: work item must contain exactly one POC outcome MUTATED`, restored, green — covers `{relative} omits the v4 POC contract`'s T1 half; (3) `linear-admission.py` L402 `--expected-source` mutated → full contract test exit 1 "clean Linear admission failed: linear admission: planning comparator returned invalid output", restored, green — covers `self-adoption omits Linear cutover boundary`'s T1 half; (4) `profile-contract-loader.py` L697 `if validate_admission` → `if True` → `profile-contract-loader.test.py` exit 1 "profile contract: new admission cannot contain Acceptance evidence with canonical criteria", restored, green — covers `package README omits admission boundary`, and this exact message independently confirms the bounce fix's test-ordering repair actually works (this is the *specific* evidence the bounce commit said was previously masked). Working tree clean after every restore. `git diff origin/main...HEAD` audit: exactly 2 site-key messages removed relative to `origin/main` (`package README omits admission boundary`, `package README omits mod boundary`), both with replacements in the mutation replay tables; the 5 bounce-restored sites (`continuation is missing`, `continuation omits provider engage behavior`, `adopter omits static Local Profile marker`, `kernel backlog exit bar is missing`, `continuation omits doc trigger`) produce **zero diff hunks** against `origin/main` — confirmed byte-identical, not just message-identical.
- DONE: Confirm the receipt's `equivalence_instrument`/`equivalence_instrument_failure` still describe the instrument at this tip, and the loader accepts the receipt at validation.
  The receipt still cited candidate `99a0c826` and a mutation against `kernel backlog exit bar is missing`, a site the bounce restored to a phrase pin (no longer tier-1) — both stale. Updated both fields in place: candidate reference now `370f02ba`, and the 4th mutation now cites the `package README omits admission boundary` triple from the item above (still tier-1 after the bounce), with a one-line note on what was superseded and why. `python3 kc-dev-flow/scripts/profile-contract-loader.py --work-item <this entity> --format json < /dev/null` → exit 0, `workflow_stage: validation`, `logical_stage: verify-deliver`, `next_workflow_stage: done` — receipt accepted.
- DONE (delivery still blocked, but the earlier blocker is resolved): Delivery readiness for the exact revision.
  `git merge-tree --write-tree origin/main HEAD` → exit 0, single tree SHA, no conflict markers — confirmed clean independently (this coordinator said they checked; I re-ran it myself rather than trusting the claim, no rebase/merge performed, non-mutating). Linear `branchName` re-verified via GraphQL: exact match `feature/dev-51-replace-kc-dev-flow-contract-test-phrase-pins-with-behaviour`; close line `Fixes DEV-51`. Absolute-claim scan (`exactly|only|always|never|cannot|byte-for-byte`) over the diff's added lines and all three fix commit messages (`392bd3b6`, `99a0c826`, `370f02ba`): the only new-text hit is a code comment ("mechanism citations hold only under a full run") that names its own enforcement point in the same sentence (`run()` gating) — no violation. Wording-only/tier-2-under-the-Captain's-ruling count: **23** (18 original + 5 bounce-restored), independently recomputed from the corrected tier table and confirmed against the entity's own stated "grows from 18 to 23"; added a cross-reference note in `## Tier-2 final disposition` so a reader stopping at that table's stale "18" is redirected rather than misled. The DEV-52 tier-2-negative-result finding was already recorded; added the new systemic finding (quick-mode `--ablation-check` skips mechanism suites) as a second finding next to it, needing a **new** follow-up Linear issue — no existing DEV issue names it (searched "ablation": DEV-11, DEV-12, DEV-46, DEV-59, none fit) — not created without Captain authorization. The DEV-60 mapping caveat from the prior cycle stands unchanged (its admitted scope doesn't itself name provider-result truncation).

### Summary

Re-verified all three checklist items at the new exact candidate `370f02ba` after the prior evidence at `99a0c826` was voided by the merge and bounce fix. Both exact commands ran clean with stdin closed (contract test PASS 44.6s; full ablation PASS 2m07s, 64/0 mutants), and 4 replayed triples — one per mechanism, each on a site the bounce table still lists as converted — reddened and restored exactly as recorded, including one (`profile-contract-loader.py`'s admission-boundary mutation) whose message independently confirms the bounce fix's test-reordering repair is real. The diff audit against `origin/main` found only 2 removed pins (both tabled) and confirmed the 5 bounce-restored sites are byte-identical to `origin/main`, not merely message-identical. The equivalence-instrument receipt fields were stale (referencing the voided candidate and a since-restored site) and are now corrected and loader-accepted. The merge-tree conflict that blocked the prior cycle is resolved (confirmed independently, not just trusted); delivery readiness is otherwise unchanged from the prior report except the wording-only count is now stated as 23 with a pointer to reasons, and a new systemic finding (quick-mode ablation blindness to document-prose mutations) is recorded needing a follow-up issue.

**Authorization requested from the Captain:** (1) explicit authorization to push `370f02ba` to `feature/dev-51-replace-kc-dev-flow-contract-test-phrase-pins-with-behaviour` and open the Draft PR with close line `Fixes DEV-51` — the merge-tree blocker is gone and no other blocker was found; (2) whether to open a new follow-up Linear issue for the quick-mode-ablation systemic finding, and whom to route the DEV-60 truncated-provider-result mapping to if DEV-60 is not the intended target.

## Post-approval comment pass (2026-09-02T17:27:13Z)

Captain asked that every comment in scripts/kc-dev-flow-contract-test.py be necessary. The five-line comment DEV-51 added before the continuation-policy pins narrated the task and pointed at this work item; the one fact it carried (`--ablation-check` skips the mechanism sub-suites run() shells out to) now sits as two lines at the flag's definition. No other comment changed. New exact candidate 3d202e1f5ef13f6526ce9e233eaf021b7df37f99, pushed to the PR head: contract test PASS, full ablation PASS (64 rejected, 0 survivors). The validation ruling for 370f02ba is carried to this comment-only revision by the First Officer's re-run of the same two suites; no assertion changed.
