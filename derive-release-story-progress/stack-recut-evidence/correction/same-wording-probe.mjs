...
const here = resolve(process.env.RECUT_PROOF_DIR ?? dirname(fileURLToPath(import.meta.url))); mkdirSync(here,{recursive:true});
const root = process.env.RECUT_SNAPSHOT_ROOT ?? resolve(here, '..');
const layer=Number(process.env.RECUT_LAYER ?? 2);
const plugin = resolve(root, `layer-${layer}/kc-journey-map`);
...
const evidence={layer,tree:spawnSync('git',['-C',resolve(plugin,'..'),'write-tree'],{encoding:'utf8'}).stdout.trim(),read_file_sha256:hash(resolve(plugin,'lib/read.mjs')),kind:'refusal',seam:'real server PATCH -> journey-read CLI --out -> YAML identity',cases:[]};
try {
...
process.exitCode=evidence.cases.every(c=>c.cli_exit===0 && c.identity_preserved && c.source_preserved)?0:1;
