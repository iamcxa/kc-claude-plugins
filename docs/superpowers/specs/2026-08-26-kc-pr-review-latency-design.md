...
predecessor. Ancestry is one persistent batch traversal of exact-SHA raw commit `parent` headers:
10,000 commits, 1 MiB per commit, and two seconds by default, with hard maxima of 100,000, 4 MiB,
and 30 seconds; exhaustion selects `initial`. Other operational object commands use a private config/ref/graft/shallow-free Git
