...
`sprint-readiness: ready` as local Spacedock execution mechanics, not planning
evidence. Its
...
then, so the admitted execution set is `spacedock status --workflow-dir docs/dev
