...
| Compiled script fails in CI but passes locally | Playwright `fill`/`click` actionability differs in headless Chrome. Mapped visibility uses the generated shared DOM classifier, not raw `is visible` or snapshot fallback; non-CSS mapping DSL needs `css_selector`. Fill/click may still need the documented login-flow patches. |
| Compile fails with missing mapped DOM identity | A mapped `role=`/`text=`/other non-CSS visibility locator has no `css_selector`. Add a literal CSS selector that identifies the same element; do not preserve the old snapshot fallback. |
| Visibility exits terminal with `raw_multi_match` or `multiple_rendered` | Make the mapping unique. Use `visibility_policy: retained-zero-rect` only after evidence proves the exact one-rendered-plus-zero-rect-ghost signature. Generated metrics retain the result class, effective selector/policy, counts, attempts/elapsed, and bounded candidate evidence. |
| Compile fails with `banned selector class '…'` | The mapping uses a form the selector policy rejects (`>> nth=N`, `:has-text(`, or a `find role\|text\|label\|testid …` subcommand chain stored as a `selector:` value) **on an element this flow resolves**. Fix the mapping per `CLAUDE.md` § Selector Priority. If it is pre-existing debt, record it in the selector baseline — `node bin/e2e-selector-baseline.js <mapping.yaml> > .claude/e2e/selector-baseline.tsv` — which is a reviewable edit, not a flag the compiler offers. Banned selectors elsewhere in the same mapping print as warnings and do not block. |
| `combobox` mapping has no concrete DOM identity | `role=combobox[name="..."]` is non-CSS locator DSL, so mapped visibility requires `css_selector`. Add a stable test ID or literal CSS attribute selector for the intended control; do not widen it to a role-only DOM selector. `find role combobox --name "..."` as a stored `selector:` is banned (subcommand chain — CLASS 5), and `role=combobox >> nth=0` is banned for the nth chord (CLASS 2). See `e2e-pipeline/compiler/lib/selector-policy.js`. |
| `Cannot find module` on first run | npm dependencies not installed. Run `npm install` in the e2e-pipeline plugin directory. |
| `Click <element(param=val)> on <loc>` refused | Executor divergence, not a typo. The parameterized element reference is an **`/e2e-test` agent-runner** form — it substitutes `${param}` into the mapping selector, and `agents/e2e-test-runner.md` documents it as supported *there*. `/e2e-compile` substitutes selector parameters in `expect:` entries only, never on a click action, so it refuses the form at compile time rather than silently dropping the parameters **and** the ` on <page>` qualifier and emitting `agent-browser click '[data-testid="row-${id}"]'` with the braces intact (#189). Click an element whose selector is literal, or assert the parameterized element through an `expect:` entry. |
