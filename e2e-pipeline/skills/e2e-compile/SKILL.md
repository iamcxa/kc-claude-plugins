...
| `combobox` pattern too broad in a11y grep | `role=combobox[name="..."]` converts to a grep pattern on the accessible name. If the element has no accessible name, the pattern falls back to `combobox` which matches any combobox — acceptable when the page has only one; add a `name`/`aria-label` or `data-testid` to the element for disambiguation. `find role combobox --name "..."` as a `selector:` value is banned (subcommand chain — CLASS 5). `role=combobox >> nth=0` is banned for the `>> nth=0` chord specifically (CLASS 2), not for `role=combobox` — see `e2e-pipeline/compiler/lib/selector-policy.js`. |
