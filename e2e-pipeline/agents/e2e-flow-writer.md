...
9. **NEVER use `manual: true`** — It is not part of the flow schema. For non-browser actions (CLI, API calls, scripts), use `Execute external` with an `execute:` block. For analytics/tracing verification (PostHog, Langfuse, Sentry), use `Verify external` with a `verify:` block. These are the supported checkpoint step types. If a checkpoint is genuinely human-only and cannot be represented as `Execute external` or `Verify external`, use a single `expect` item with `not_automated: "<reason>"`. Do not write `manual ...` as a plain expect string.
