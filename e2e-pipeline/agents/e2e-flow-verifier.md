...
9. **Never use `has-text()`** — Broken in agent-browser, causes timeout. Use the native `find role button --name "<label>"` subcommand form instead. Do NOT use the banned Playwright attr syntax `role=button[name="..."]` — this is BANNED (see selector priority rules in CLAUDE.md).
