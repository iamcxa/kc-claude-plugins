...
| **Health table** | Base 3 rows (always present): API failures, Console errors, Trace. Walkthrough extension: +2 rows when step-log cross-reference data is available (see below). |

**Walkthrough Health table** (5 rows):
...
The extra 2 rows are only present when the walkthrough's trace-analyzer returns enhanced data (i.e., `step_log_path` was provided). For test and flow, always use the base 3-row table.
