...
Evidence validation must constrain stdout shape, not merely search for a plausible substring. A URL probe should accept one clean supported URL line beginning at offset zero; a snapshot probe should require at least one recognizable accessibility-tree role line. Error prose can contain strings such as `websocket://`, so broad scheme matching is not evidence validation. Failure diagnostics must use the same named browser session as the failed action or assertion. **Applies to**: CLI wrappers whose tools may print protocol-like diagnostics while exiting successfully.
