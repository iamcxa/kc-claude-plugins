...
# DO NOT EDIT -- regenerate with: e2e-compile test-login
# Source: compiler/test/fixtures/simple-flow.yaml
# Mapping: compiler/test/fixtures/test-app.yaml
# Generated: 2026-03-17T18:15:13.378Z
# SHA-256: f8ce99edbffef0cb54889a33323279b224eaa5ffd4567a10305523412b679617
...
_emit_junit() {
  ...
  done
  {
    ...
    printf '<testsuites>\n'
    printf '  <testsuite name="test-login" tests="%s" failures="%s" skipped="%s" time="%s" timestamp="%s">\n' \
    ...
    for _i in "${!_STEP_NAMES[@]}"; do
      ...
      if [ "$_sresult" = "skip" ]; then
        printf '    <testcase classname="test-login" name="%s" time="%s"><skipped/></testcase>\n' \
          "$_sname" "$_stime"
      elif [ "$_sresult" = "fail" ]; then
        printf '    <testcase classname="test-login" name="%s" time="%s"><failure message="%s"/></testcase>\n' \
          "$_sname" "$_stime" "$_sfail"
      else
        printf '    <testcase classname="test-login" name="%s" time="%s"/>\n' \
          "$_sname" "$_stime"
      fi
    done
    printf '  </testsuite>\n'
...
_emit_metrics() {
  ...
  {
    printf '{"flow":"' ; printf '%s' "test-login" ; printf '",'
...
echo "[1/6] navigate-to-login: Navigate to /login"
_STEP_START=$SECONDS
_retry=0
_step_ok=true
while true; do
...
echo "[2/6] fill-email: Fill email_input with 'test@example.com' on login"
_STEP_START=$SECONDS
_retry=0
_step_ok=true
while true; do
  agent-browser fill '[role="textbox"][aria-label="Email"]' 'test@example.com' && break
  _retry=$((_retry + 1))
  if [ "$_retry" -ge "$RETRIES" ] || [ "$RETRIES" -eq 0 ]; then
    _step_ok=false
    break
  ...
  _HAD_RETRIES=true
  echo "RETRY [$_retry/$RETRIES]: fill-email"
  sleep 2
done
_elapsed=$(( SECONDS - _STEP_START ))
if [ "$_step_ok" = "true" ]; then
  _STEP_NAMES+=("fill-email")
  _STEP_RESULTS+=("pass")
  _STEP_FAILURES+=("")
  _STEP_TIMES+=("$_elapsed")
else
...
_poll_visible '[role="textbox"][aria-label="Email"]' "fill-email" "${WAIT_TIMEOUT:-10}" "" || _handle_failure "fill-email" "email_input not visible after "${WAIT_TIMEOUT:-10}"s"
...
echo "[4/6] take-snapshot: Take snapshot"
agent-browser snapshot
_STEP_NAMES+=("take-snapshot")
_STEP_RESULTS+=("pass")
_STEP_FAILURES+=("")
_STEP_TIMES+=("0")

echo "[5/6] wait-for-load: Wait 2"
sleep 2
_STEP_NAMES+=("wait-for-load")
_STEP_RESULTS+=("pass")
_STEP_FAILURES+=("")
_STEP_TIMES+=("2")

echo "[6/6] verify-external: Verify external"
echo "SKIP: verify-external -- external verification (no human in CI)"
_STEP_NAMES+=("verify-external")
_STEP_RESULTS+=("skip")
_STEP_FAILURES+=("")
_STEP_TIMES+=("0")
...
if [ "$_HAD_RETRIES" = "true" ]; then
  echo "PASS (FLAKY): test-login (6/6 steps, 1 skipped)"
else
  echo "PASS: test-login (6/6 steps, 1 skipped)"
fi
exit 0