...
// #149. The receipt used to reach `verified` on the strength of the init script
// having attached, which is evidence about the recorder and says nothing about
// whether the browser populated the origin from the profile. agent-browser 0.32
// snapshot mode drops Local Storage, so a run against a pre-authenticated profile
// behaved as a logged-out visitor start to finish while the artifact stayed green.

test('snapshot profile that carried origin state records the observation and verifies', function(t) {
  const fixture = setup(t, { profileMode: 'snapshot' });
  fixture.env.E2E_FIXTURE_PROFILE_STORAGE = JSON.stringify({
    origin: 'https://application.example.test',
    local_storage_keys: 4,
  ...
  const receipt = JSON.parse(fs.readFileSync(fixture.receipt, 'utf8'));
  assert.equal(receipt.first_navigation.status, 'verified');
  assert.equal(receipt.first_navigation.profile_storage.status, 'observed');
  assert.equal(receipt.first_navigation.profile_storage.local_storage_keys, 4);
  assert.equal(receipt.first_navigation.profile_storage.cookie_count, 2);
  assert.equal(
    receipt.first_navigation.post.profile_storage.origin,
    'https://application.example.test'
  );
});

test('inert snapshot profile fails closed instead of reporting verified', function(t) {
  const fixture = setup(t, { profileMode: 'snapshot' });
  fixture.env.E2E_FIXTURE_PROFILE_STORAGE = JSON.stringify({
    origin: 'https://application.example.test',
  ...
  assert.match(receipt.error, /profile snapshot carried no origin state/);
  assert.notEqual(receipt.first_navigation.status, 'verified');
});

test('unreadable origin storage is not reported as an observation of emptiness', function(t) {
  const fixture = setup(t, { profileMode: 'snapshot' });
  fixture.env.E2E_FIXTURE_PROFILE_STORAGE = JSON.stringify({
    origin: '',
    local_storage_keys: null,
    session_storage_keys: null,
    cookie_count: null,
  });

  const result = runOpen(fixture, 'https://application.example.test/opaque');

  // An opaque origin throws on localStorage access. That is absent evidence, not
  // evidence of absence, so it must not trip the inert-snapshot refusal above —
  // and it must not be laundered into a positive observation either.
  assert.equal(result.status, 0, result.stderr);
  const receipt = JSON.parse(fs.readFileSync(fixture.receipt, 'utf8'));
  assert.equal(receipt.first_navigation.profile_storage.status, 'unreadable');
  assert.equal(receipt.first_navigation.profile_storage.local_storage_keys, null);
});

test('empty origin storage under a persistent-path profile stays verified', function(t) {
  const fixture = setup(t);
  fixture.env.E2E_FIXTURE_PROFILE_STORAGE = JSON.stringify({
    origin: 'https://application.example.test',
    local_storage_keys: 0,
  ...
  const result = runOpen(fixture, 'https://application.example.test/fresh');

  // The refusal is scoped to `verified-snapshot`, the mode that copies a profile
  // before launch and therefore the mode that can silently drop its contents. A
  // persistent-path run has no copy step, and a genuinely fresh profile legitimately
  // has nothing at the origin — refusing here would fail runs that never had state
  // to carry.
  assert.equal(result.status, 0, result.stderr);
  const receipt = JSON.parse(fs.readFileSync(fixture.receipt, 'utf8'));
  assert.equal(receipt.profile_mode, 'persistent-path');
  assert.equal(receipt.first_navigation.status, 'verified');
  assert.equal(receipt.first_navigation.profile_storage.local_storage_keys, 0);
});
