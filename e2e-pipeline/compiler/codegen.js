...
function generateRuntimeSupport() {
  var lines = [
    ...
    '  fi',
    '  if ! printf \'%s\\n\' "$_snapshot" | sed -n \'1p\' | grep -Eq \'^[[:space:]-]*(document|generic|main|navigation|banner|contentinfo|region|form|heading|paragraph|text|StaticText|button|link|textbox|searchbox|checkbox|radio|switch|combobox|listbox|option|tab|tabpanel|menu|menuitem|dialog|alert|status|img|list|listitem|table|row|cell|grid|tree|treeitem|slider|spinbutton|separator)([[:space:]:]|$)\'; then return 2; fi',
    '  printf \'%s\\n\' "$_snapshot"',
    '}',
    '',
    '# Status-safe URL capture. A valid browser URL is exactly one http(s) line.',
    '_capture_url() {',
    '  local _session="$' + '{1:-}"',
    '  local _url',
    '  if [ -n "$_session" ]; then',
    ...
    '  fi',
    "  case \"$_url\" in *$'\\n'*) return 2 ;; esac",
    '  if ! printf \'%s\\n\' "$_url" | grep -Eq \'^https?://[^[:space:]]+$\'; then return 2; fi',
    '  printf \'%s\\n\' "$_url"',
    '}',
    '',
    '_poll_url_contains() {',
